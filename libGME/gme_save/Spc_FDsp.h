...
// Setup
    ...
    void set_interpolation_level( int level = 0 );
...
inline int Spc_FDsp::sample_count() const { return m.out - m.out_begin; }

inline int Spc_FDsp::read( int addr ) const
{
	assert( (unsigned) addr < register_count );
	return m.regs [addr];
}

inline void Spc_FDsp::update_voice_vol( int addr )
{
...
inline void Spc_FDsp::write( int addr, int data )
{
	assert( (unsigned) addr < register_count );
...
inline void Spc_FDsp::disable_surround( bool disable )
{
	m.surround_threshold = disable ? 0 : -0x4000;
}

//inline void Spc_FDsp::mute_voices( int mask ) { m.mute_mask = mask; }
