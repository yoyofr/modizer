...
static BOOST::uint8_t const initial_regs [Spc_FDsp::register_count] =
{
...
void Spc_FDsp::set_output( sample_t out [], int size )
{
...
inline void Spc_FDsp::init_counter()
{
...
inline void Spc_FDsp::run_counter( int i )
{
	int n = m.counters [i];
	if ( !(n-- & 7) )
		n -= 6 - i;
...
void Spc_FDsp::run( int clock_count )
{
...
void Spc_FDsp::set_interpolation_level( int level ) { 
    m.interpolation_level = level; 
}


void Spc_FDsp::mute_voices( int mask )
{
	m.mute_mask = mask;
...
void Spc_FDsp::init( void* ram_64k )
{
...
void Spc_FDsp::soft_reset_common()
{
...
void Spc_FDsp::soft_reset()
{
	REG(flg) = 0xE0;
	soft_reset_common();
}

void Spc_FDsp::load( uint8_t const regs [register_count] )
{
...
void Spc_FDsp::reset() { load( initial_regs ); }
