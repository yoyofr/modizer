...
bool OpenVGMFile(const char* FileName)
{
	...
	if (VGMHead.lngExtraOffset)
	{
		...
		if (TempLng > sizeof(VGM_HDR_EXTRA))
			TempLng = sizeof(VGM_HDR_EXTRA);
	...
	case 0x00:
		if (! (ChipID & 0x80))
		{
			...
			/*
				...
				T6W28			 0x4000, 0x01, 0x04, ????,  FALSE, 8, ????		05	0F	??
			*/
		}
		else
		{
...
static UINT16 GetChipVolume(VGM_HEADER* FileHead, UINT8 ChipID, UINT8 ChipNum)
{
	...
	// ChipNum: chip volume divider (number of used chips)
	//		Bit 7 - Is Second Chip
	...
	{	0x80, 0x200/*0x155*/, 0x100, 0x100, 0x180, 0xB0, 0x100, 0x80,	// 00-07
		...
		0x80, 0xE0/*0xCD*/, 0x40, 0x60, 0x80, 0x100, 0x11E, 0x1C0,		// 10-17
	...
	case 0x00:	// SN76496
		...
		if (GetChipClock(&VGMHead, (ChipNum & 0x80) | 0x00, NULL) & 0x80000000)
			ChipNum = (ChipNum & 0x80) | 0x01;
		break;
	case 0x18:	// OKIM6295
		// CP System 1 patch
	...
	case 0x00:	// Start Chips
		...
		if (VGMHead.lngHzPSG)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip ++)
			{
				...
				if (! UseFM)
				{
					...
					CAA->Volume = GetChipVolume(&VGMHead, 0x00, (CurChip << 7) | ChipCnt);
		...
		if (VGMHead.lngHzYM2413)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip ++)
			{
				...
				if (! UseFM)
				{
					...
					CAA->Volume = GetChipVolume(&VGMHead, 0x01, (CurChip << 7) | ChipCnt);
		...
		if (VGMHead.lngHzYM2612)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip ++)
			{
				...
				CAA->Volume = GetChipVolume(&VGMHead, 0x02, (CurChip << 7) | ChipCnt);
				AbsVol += CAA->Volume;
			}
		}
		if (VGMHead.lngHzYM2151)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip ++)
			{
				...
				CAA->Volume = GetChipVolume(&VGMHead, 0x03, (CurChip << 7) | ChipCnt);
				AbsVol += CAA->Volume;
			}
		}
		if (VGMHead.lngHzSPCM)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip ++)
			{
				...
				CAA->Volume = GetChipVolume(&VGMHead, 0x04, (CurChip << 7) | ChipCnt);
				AbsVol += CAA->Volume;
			}
		}
		if (VGMHead.lngHzRF5C68)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip ++)
			{
				...
				CAA->Volume = GetChipVolume(&VGMHead, 0x05, (CurChip << 7) | ChipCnt);
				AbsVol += CAA->Volume;
			}
		}
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip ++)
			{
				...
				CAA->SmpRate = device_start_ym2203(CurChip, ChipClk,
												   ChipOpts[CurChip].YM2203.SpecialFlags & 0x01,
													...
													&CAA->Paired->SmpRate);
				CAA->Volume = GetChipVolume(&VGMHead, 0x06, (CurChip << 7) | ChipCnt);
				AbsVol += CAA->Volume;
				CAA->Paired->Volume = GetChipVolume(&VGMHead, 0x86, (CurChip << 7) | ChipCnt);
				AbsVol += CAA->Paired->Volume;
			}
		}
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip ++)
			{
				...
				CAA->SmpRate = device_start_ym2608(CurChip, ChipClk,
												   ChipOpts[CurChip].YM2608.SpecialFlags & 0x01,
													...
													&CAA->Paired->SmpRate);
				CAA->Volume = GetChipVolume(&VGMHead, 0x07, (CurChip << 7) | ChipCnt);
				AbsVol += CAA->Volume;
				CAA->Paired->Volume = GetChipVolume(&VGMHead, 0x87, (CurChip << 7) | ChipCnt);
				AbsVol += CAA->Paired->Volume;
				//CAA->Volume = ChipVol;
		...
		if (VGMHead.lngHzYM2610)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip ++)
			{
				...
				CAA->SmpRate = device_start_ym2610(CurChip, ChipClk,
													...
													&CAA->Paired->SmpRate);
				CAA->Volume = GetChipVolume(&VGMHead, 0x08, (CurChip << 7) | ChipCnt);
				AbsVol += CAA->Volume;
				CAA->Paired->Volume = GetChipVolume(&VGMHead, 0x88, (CurChip << 7) | ChipCnt);
				AbsVol += CAA->Paired->Volume;
				//CAA->Volume = ChipVol;
				//CAA->Paired->Volume = ChipVol * 2;
			}
		...
		if (VGMHead.lngHzYM3812)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip ++)
			{
				...
				if (! UseFM)
				{
					...
					CAA->Volume = GetChipVolume(&VGMHead, 0x09, (CurChip << 7) | ChipCnt);
					AbsVol += CAA->Volume * 2;
				}
				else
				{
		...
		if (VGMHead.lngHzYM3526)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip ++)
			{
				...
				if (! UseFM)
				{
					...
					CAA->Volume = GetChipVolume(&VGMHead, 0x0A, (CurChip << 7) | ChipCnt);
					AbsVol += CAA->Volume * 2;
				}
				else
				{
		...
		if (VGMHead.lngHzY8950)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip ++)
			{
				...
				if (! UseFM)
				{
					...
					CAA->Volume = GetChipVolume(&VGMHead, 0x0B, (CurChip << 7) | ChipCnt);
					AbsVol += CAA->Volume;
				}
				else
				{
		...
		if (VGMHead.lngHzYMF262)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip ++)
			{
				...
				if (! UseFM)
				{
					...
					CAA->Volume = GetChipVolume(&VGMHead, 0x0C, (CurChip << 7) | ChipCnt);
					AbsVol += CAA->Volume * 2;
				}
				else
				{
		...
		if (VGMHead.lngHzYMF278B)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip ++)
			{
				...
				CAA->Volume = GetChipVolume(&VGMHead, 0x0D, (CurChip << 7) | ChipCnt);
		...
		if (VGMHead.lngHzYMF271)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip ++)
			{
				...
				CAA->Volume = GetChipVolume(&VGMHead, 0x0E, (CurChip << 7) | ChipCnt);
				AbsVol += CAA->Volume;
			}
		}
		if (VGMHead.lngHzYMZ280B)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip ++)
			{
				...
				CAA->Volume = GetChipVolume(&VGMHead, 0x0F, (CurChip << 7) | ChipCnt);
		...
		if (VGMHead.lngHzRF5C164)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip ++)
			{
				...
				CAA->Volume = GetChipVolume(&VGMHead, 0x10, (CurChip << 7) | ChipCnt);
				AbsVol += CAA->Volume * 2;
			}
		}
		if (VGMHead.lngHzPWM)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip ++)
			{
				...
				CAA->Volume = GetChipVolume(&VGMHead, 0x11, (CurChip << 7) | ChipCnt);
				AbsVol += CAA->Volume;
			}
		}
		if (VGMHead.lngHzAY8910)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip ++)
			{
				...
				CAA->Volume = GetChipVolume(&VGMHead, 0x12, (CurChip << 7) | ChipCnt);
				AbsVol += CAA->Volume * 2;
			}
		}
		if (VGMHead.lngHzGBDMG)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip ++)
			{
				...
				CAA->Volume = GetChipVolume(&VGMHead, 0x13, (CurChip << 7) | ChipCnt);
				AbsVol += CAA->Volume * 2;
			}
		}
		if (VGMHead.lngHzNESAPU)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip ++)
			{
				...
				CAA->Volume = GetChipVolume(&VGMHead, 0x14, (CurChip << 7) | ChipCnt);
				AbsVol += CAA->Volume * 2;
			}
		}
		if (VGMHead.lngHzMultiPCM)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip ++)
			{
				...
				CAA->Volume = GetChipVolume(&VGMHead, 0x15, (CurChip << 7) | ChipCnt);
				AbsVol += CAA->Volume;
			}
		}
		if (VGMHead.lngHzUPD7759)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip ++)
			{
				...
				CAA->Volume = GetChipVolume(&VGMHead, 0x16, (CurChip << 7) | ChipCnt);
				AbsVol += CAA->Volume;
			}
		}
		if (VGMHead.lngHzOKIM6258)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip ++)
			{
				...
				CAA->Volume = GetChipVolume(&VGMHead, 0x17, (CurChip << 7) | ChipCnt);
				AbsVol += CAA->Volume;
			}
		}
		if (VGMHead.lngHzOKIM6295)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip ++)
			{
				...
				CAA->Volume = GetChipVolume(&VGMHead, 0x18, (CurChip << 7) | ChipCnt);
				AbsVol += CAA->Volume;
			}
		}
		if (VGMHead.lngHzK051649)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip ++)
			{
				...
				CAA->Volume = GetChipVolume(&VGMHead, 0x19, (CurChip << 7) | ChipCnt);
				AbsVol += CAA->Volume;
			}
		}
		if (VGMHead.lngHzK054539)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip ++)
			{
				...
				CAA->Volume = GetChipVolume(&VGMHead, 0x1A, (CurChip << 7) | ChipCnt);
				AbsVol += CAA->Volume;
			}
		}
		if (VGMHead.lngHzHuC6280)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip ++)
			{
				...
				CAA->Volume = GetChipVolume(&VGMHead, 0x1B, (CurChip << 7) | ChipCnt);
				AbsVol += CAA->Volume;
			}
		}
		if (VGMHead.lngHzC140)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip ++)
			{
				...
				CAA->Volume = GetChipVolume(&VGMHead, 0x1C, (CurChip << 7) | ChipCnt);
				AbsVol += CAA->Volume;
			}
		}
		if (VGMHead.lngHzK053260)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip ++)
			{
				...
				CAA->Volume = GetChipVolume(&VGMHead, 0x1D, (CurChip << 7) | ChipCnt);
				AbsVol += CAA->Volume;
			}
		}
		if (VGMHead.lngHzPokey)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip ++)
			{
				...
				CAA->Volume = GetChipVolume(&VGMHead, 0x1E, (CurChip << 7) | ChipCnt);
				AbsVol += CAA->Volume;
			}
		}
		if (VGMHead.lngHzQSound)
		{
			...
			for (CurChip = 0x00; CurChip < ChipCnt; CurChip ++)
			{
				...
				CAA->Volume = GetChipVolume(&VGMHead, 0x1F, (CurChip << 7) | ChipCnt);
				AbsVol += CAA->Volume;
			}
		}
		...
		else
		{
			...
			case 0xB7:	// OKIM6258 write
				...
				if (VGMHead.lngHzOKIM6258)
				{
					...
					{
						ChipAudio[CurChip].OKIM6258.Resampler = 0x01;
						ChipAudio[CurChip].OKIM6258.SmpP = 9;
						ChipAudio[CurChip].OKIM6258.SmpLast = 0;
						ChipAudio[CurChip].OKIM6258.SmpNext = 0;
					}*/
				}
				VGMPos += 0x03;
				break;
