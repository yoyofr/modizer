...
//-----------------------------------------------------
{
	...
	case 0x0F:	command = (param <= (UINT)((m_nType & (MOD_TYPE_XM|MOD_TYPE_MT2)) ? 0x1F : 0x20)) ? CMD_SPEED : CMD_TEMPO;
			...
			if (noTempoCommand) command=CMD_SPEED; //NoiseTracker has no tempo command. Example : Nebulos / Audiomonster
			break;
...
//---------------------------------------------------------------
{
	...
	//check if Noisetracker : in this case there is no tempo command
	if (IsMagic(s,"M.K.")||IsMagic(s, "N.T.")) {
		//check the special byte used for loop pos by Noisetracker
		//usual historical value is 0x7F for protracker
		if (lpStream[951]!=0x7F) noTempoCommand=1;
	}
