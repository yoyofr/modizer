...
static int read_sample_data(int32 flags, struct timidity_file *tf, int bits, int samples, int frames, tim_sample_t **sdata);
...
static int import_wave_load(char *sample_file, Instrument *inst)
{
	...
	for(;;) {
		...
		{
			int				frames;
			tim_sample_t		*sdata[MAX_SAMPLE_CHANNELS];
			...
			for(i = 0; i < samples; i++)
			{
				inst->sample[i].data = sdata[i] = (tim_sample_t *)safe_malloc(sizeof(tim_sample_t) * frames);
...
static int read_AIFFSoundData(struct timidity_file *tf, Instrument *inst, AIFFCommonChunk *common)
{
	...
	for(i = 0; i < samples; i++)
	{
		sample[i].data = sdata[i] = (tim_sample_t *)safe_malloc(sizeof(tim_sample_t) * common->numSampleFrames);
...
static int read_sample_data(int32 flags, struct timidity_file *tf, int bits, int channels, int frames, tim_sample_t **sdata)
{
...
static void initialize_sample(Instrument *inst, int frames, int sample_bits, int sample_rate)
{
	...
	for(i = 0; i < samples; i++)
	{
		...
		sample->tim_sample_type = SF_SAMPLETYPE_MONO;
