...
bool pxtnService::_moo_PXTONE_SAMPLE( void *p_data )
{
    ...
    for( int32_t ch = 0; ch < _dst_ch_num; ch++ )
    {
        ...
        for( int32_t u = 0; u < _unit_num ; u++ ) {
            ...
            //YOYOFR
            if (mute_mask&(1<<u)) _units[u]->last_smpl=0;
            else _units [ u ]->Tone_Supple(     _moo_group_smps, ch, _moo_time_pan_index );
            //YOYOFR
            
            //TODO:  MODIZER changes start / YOYOFR
            if (ch==0) {
                int64_t ofs_start=m_voice_current_ptr[u];
                int64_t ofs_end=(m_voice_current_ptr[u]+smplIncr);
                for (;;) {
                    m_voice_buff[u][(ofs_start>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)&(SOUND_BUFFER_SIZE_SAMPLE*4*2-1)]=LIMIT8((_units [ u ]->last_smpl>>7));
...
bool pxtnService::Moo( void* p_buf, int32_t  size )
{
    ...
    //YOYOFR
    {
        ...
        for( smp_w = 0; smp_w < smp_num; smp_w++ )
        {
            ...
            if (p_buf) for( int ch = 0; ch < _dst_ch_num; ch++, p16++ ) *p16 = sample[ ch ];
        }
        for( ;          smp_w < smp_num; smp_w++ )
        {
            if (p_buf) for( int ch = 0; ch < _dst_ch_num; ch++, p16++ ) *p16 = 0;
        }
    }
    ...
    for( int32_t u = 0; u < _unit_num ; u++ ) {
        //YOYOFR
        if (_units[u]->get_played() && !(mute_mask&(1<<u))) {
            int32_t vol=0;
            const pxtnWoice *_p_woice=_units[u]->get_woice();
            for( int32_t v = 0; v < _p_woice->get_voice_num(); v++ ) {
                pxtnVOICETONE*           p_vt = _units[u]->get_tone(v);//&_vts                 [ v ];
                const pxtnVOICEINSTANCE* p_vi = _p_woice->get_instance( v );
                
                if( p_vt->life_count > 0 )
                {
                    vol=128;
                    vol = ( vol * _units[u]->getCurrentVelocity() )   / 128;
                    vol = ( vol * _units[u]->getCurrentVolume()   )   / 128;
                    //vol =   vol * (MAXVAL(_pan_vols[ 0 ],_pan_vols[ 1 ])) /  64;
                    
                    if( p_vi->env_size ) vol = vol * p_vt->env_volume / 128;
                }
            }
            if (vol) {
