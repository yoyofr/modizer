...
static void writeConstantConstructor (std::stringstream& out, EMslSymbolType t, TPrecision prec, TIntermConstant *c, const MslStruct *structure = 0)
{
...
void writeComparison( const TString &compareOp, const TString &compareCall, TIntermBinary *node, TMslOutputTraverser* goit )
{
   MslFunction *current = goit->current;
   ...
   if ( bUseCompareCall )
   {
      ...
      if (node->getLeft())
      {
         ...
         {
            out << "float" <<  node->getRight()->getRowsCount() << "( ";
      ...
      if (node->getRight())
      {
         ...
         {
            out << "float" <<  node->getLeft()->getRowsCount() << "( ";
...
void writeFuncCall( const TString &name, TIntermAggregate *node, TMslOutputTraverser* goit, bool bGenMatrix = false, bool mangleName = false )
{
...
void writeTex( const TString &name, TIntermAggregate *node, TMslOutputTraverser* goit )
{
	TNodeArray& nodes = node->getNodes(); 
...
void TMslOutputTraverser::outputLineDirective (const TSourceLoc& line)
{
...
TMslOutputTraverser::TMslOutputTraverser(TInfoSink& i, std::vector<MslFunction*> &funcList, std::vector<MslStruct*> &sList, std::stringstream& deferredArrayInit, std::stringstream& deferredMatrixInit, ETargetVersion version, unsigned options)
: infoSink(i)
, generatingCode(true)
...
, m_UsePrecision(false) //Hlsl2Msl_VersionUsesPrecision(version))
...
void TMslOutputTraverser::traverseArrayDeclarationWithInit(TIntermDeclaration* decl)
{
	assert(decl->containsArrayInitialization());
	
	std::stringstream* out = &current->getActiveOutput();
	TType& type = *decl->getTypePointer();
	EMslSymbolType symbol_type = translateType(decl->getTypePointer());
...
bool TMslOutputTraverser::traverseDeclaration(bool preVisit, TIntermDeclaration* decl, TIntermTraverser* it)
{
	TMslOutputTraverser* goit = static_cast<TMslOutputTraverser*>(it);
	MslFunction *current = goit->current;
...
void TMslOutputTraverser::traverseSymbol(TIntermSymbol *node, TIntermTraverser *it)
{
	TMslOutputTraverser* goit = static_cast<TMslOutputTraverser*>(it);
	MslFunction *current = goit->current;
	std::stringstream& out = current->getActiveOutput();
	...
	if ( ! current->hasSymbol( node->getId()))
	{
		...
		else
		{
			...
			MslSymbol * sym = new MslSymbol( node->getSymbol().c_str(), semantic, registerSpec, node->getId(),
				translateType(node->getTypePointer()), EbpUndefined, translateQualifier(node->getQualifier()), array);
			sym->setIsGlobal(node->isGlobal());

			current->addSymbol(sym);
			if (sym->getType() == EgstStruct)
			{
				MslStruct *s = goit->createStructFromType( node->getTypePointer());
				sym->setStruct(s);
			}
...
void TMslOutputTraverser::traverseParameterSymbol(TIntermSymbol *node, TIntermTraverser *it)
{
   TMslOutputTraverser* goit = static_cast<TMslOutputTraverser*>(it);
   MslFunction *current = goit->current;
...
void TMslOutputTraverser::traverseConstant( TIntermConstant *node, TIntermTraverser *it )
{
   TMslOutputTraverser* goit = static_cast<TMslOutputTraverser*>(it);
   MslFunction *current = goit->current;
   std::stringstream& out = current->getActiveOutput();
   EMslSymbolType type = translateType( node->getTypePointer());
   MslStruct *str = 0;
   ...
   writeConstantConstructor (out, type, EbpUndefined, node, str);
}


void TMslOutputTraverser::traverseImmediateConstant( TIntermConstant *c, TIntermTraverser *it )
{
   TMslOutputTraverser* goit = static_cast<TMslOutputTraverser*>(it);
...
static bool Check2DMatrixIndex (TMslOutputTraverser* goit, std::stringstream& out, TIntermTyped* left, TIntermTyped* right)
{
...
bool TMslOutputTraverser::traverseBinary( bool preVisit, TIntermBinary *node, TIntermTraverser *it )
{
   TString op = "??";
   TMslOutputTraverser* goit = static_cast<TMslOutputTraverser*>(it);
   MslFunction *current = goit->current;
...
//				 right->getType().buildMangledName(opName);
         ...
         {
            char swiz[] = "xyzw";
            goit->visitConstant = TMslOutputTraverser::traverseImmediateConstant;
            ...
            goit->visitConstant = TMslOutputTraverser::traverseConstant;
...
//			  right->getType().buildMangledName(opName);
   ...
   case EOpIndexDirectStruct:
      {
         current->beginStatement();
         MslStruct *s = goit->createStructFromType(node->getLeft()->getTypePointer());
         if (node->getLeft())
            node->getLeft()->traverse(goit);
   ...
   case EOpMod:    op = "fmod"; infix = false; break;
	   ...
	   if (node->getOp() == EOpAssign && node->getLeft() && node->getRight()) {
		   ...
		   if (lval && lval->getOp() == EOpMatrixSwizzle) {
			   ...
			   if (n_swizzles > 1) {
				   ...
				   current->beginStatement();
				   out << "float" << n_swizzles << " " << temp_rval << " = ";
				   rval->traverse(goit);			   
				   current->endStatement();
			   }
...
bool TMslOutputTraverser::traverseUnary( bool preVisit, TIntermUnary *node, TIntermTraverser *it )
{
   TString op("??");
   TMslOutputTraverser* goit = static_cast<TMslOutputTraverser*>(it);
   MslFunction *current = goit->current;
   ...
   {
   case EOpNegative:
       if (node->getTypePointer()->isMatrix())
       {
           current->addLibFunction(EOpNegative);
           op = "xll_negative";
           funcStyle = true; prefix = true;

       }
       else
       {
           op = "-";
           funcStyle = false; prefix = true;
       }
       break;
   ...
   case EOpInverseSqrt:    setupUnaryBuiltInFuncCall ( "rsqrt", node, op, funcStyle, prefix, goit ); break;
   ...
   case EOpLength:
       current->addLibFunction(EOpLength);
       op = "length";  funcStyle = true; prefix = true; break;
   case EOpNormalize:
       current->addLibFunction(EOpNormalize);
       op = "normalize";  funcStyle = true; prefix = true;
       break;
   case EOpDPdx:
...
bool TMslOutputTraverser::traverseSelection( bool preVisit, TIntermSelection *node, TIntermTraverser *it )
{
	TMslOutputTraverser* goit = static_cast<TMslOutputTraverser*>(it);
	MslFunction *current = goit->current;
	std::stringstream& out = current->getActiveOutput();
...
bool TMslOutputTraverser::traverseAggregate( bool preVisit, TIntermAggregate *node, TIntermTraverser *it )
{
   TMslOutputTraverser* goit = static_cast<TMslOutputTraverser*>(it);
   MslFunction *current = goit->current;
   std::stringstream& out = current->getActiveOutput();
...
//		  goit->outputLineDirective (node->getLine());
   ...
   case EOpFunction:
      {
         MslFunction *func = new MslFunction( node->getPlainName().c_str(), node->getName().c_str(),
                                                translateType(node->getTypePointer()), EbpUndefined,
   ...
   case EOpConstructFloat:
       if (node->getNodes().size() == 1) {
           current->addLibFunction(EOpConstructFloat);
           writeFuncCall( "xll_construct_float", node, goit);
       } else {
           writeFuncCall( "float", node, goit);
       }
       return false;
   case EOpConstructVec2:
       if (node->getNodes().size() == 1) {
           current->addLibFunction(EOpConstructVec2);
           writeFuncCall( "xll_construct_float2", node, goit); return false;
       } else {
           writeFuncCall( "float2", node, goit);
       }
       return false;
   case EOpConstructVec3:
       if (node->getNodes().size() == 1) {
           current->addLibFunction(EOpConstructVec3);
           writeFuncCall( "xll_construct_float3", node, goit); return false;
       } else {
           writeFuncCall( "float3", node, goit);
       }
       return false;
   case EOpConstructVec4:
       if (node->getNodes().size() == 1) {
           current->addLibFunction(EOpConstructVec4);
           writeFuncCall( "xll_construct_float4", node, goit);
       } else {
           writeFuncCall( "float4", node, goit);

       }
       return false;
   case EOpConstructBool:  writeFuncCall( "bool", node, goit); return false;
   case EOpConstructBVec2: writeFuncCall( "bool2", node, goit); return false;
   case EOpConstructBVec3: writeFuncCall( "bool3", node, goit); return false;
   case EOpConstructBVec4: writeFuncCall( "bool4", node, goit); return false;
   case EOpConstructInt:   writeFuncCall( "int", node, goit); return false;
   case EOpConstructIVec2: writeFuncCall( "int2", node, goit); return false;
   case EOpConstructIVec3: writeFuncCall( "int3", node, goit); return false;
   case EOpConstructIVec4: writeFuncCall( "int4", node, goit); return false;

   case EOpConstructMat2x2:
       if (node->getNodes().size() == 1) {
           current->addLibFunction(EOpConstructMat2x2);
           writeFuncCall( "xll_construct_float2x2", node, goit);
       } else {
           writeFuncCall( "float2x2",   node, goit);
       }
       return false;
       
   case EOpConstructMat2x3:  writeFuncCall( "float2x3", node, goit); return false;
   case EOpConstructMat2x4:  writeFuncCall( "float2x4", node, goit); return false;

   case EOpConstructMat3x2:  writeFuncCall( "float3x2", node, goit); return false;
   case EOpConstructMat3x3:
       if (node->getNodes().size() == 1) {
           current->addLibFunction(EOpConstructMat3x3);
           writeFuncCall( "xll_construct_float3x3", node, goit);
       } else {
           writeFuncCall( "float3x3",   node, goit);
       }
       return false;
   case EOpConstructMat3x4:  writeFuncCall( "float3x4", node, goit); return false;

   case EOpConstructMat4x2:  writeFuncCall( "float4x2", node, goit); return false;
   case EOpConstructMat4x3:  writeFuncCall( "float4x3", node, goit); return false;
   case EOpConstructMat4x4:
       if (node->getNodes().size() == 1) {
           current->addLibFunction(EOpConstructMat4x4);
           writeFuncCall( "xll_construct_float4x4", node, goit);
       } else {
           writeFuncCall( "float4x4",   node, goit);
       }
       return false;
   ...
   case EOpPow:           writeFuncCall( "xll_pow", node, goit, true); return false;

   case EOpAtan2:         writeFuncCall( "atan2", node, goit, true); return false;
   ...
   case EOpTex2D:
      if (argCount == 2)
      {  
          writeTex( "metal_texture", node, goit);
      }
      else
      {
...
bool TMslOutputTraverser::traverseLoop( bool preVisit, TIntermLoop *node, TIntermTraverser *it )
{
   TMslOutputTraverser* goit = static_cast<TMslOutputTraverser*>(it);
   MslFunction *current = goit->current;
   std::stringstream& out = current->getActiveOutput();
...
bool TMslOutputTraverser::traverseBranch( bool preVisit, TIntermBranch *node,  TIntermTraverser *it )
{
   TMslOutputTraverser* goit = static_cast<TMslOutputTraverser*>(it);
   MslFunction *current = goit->current;
   std::stringstream& out = current->getActiveOutput();
...
MslStruct *TMslOutputTraverser::createStructFromType (TType *type)
{
   MslStruct *s = 0;
   ...
   if ( structMap.find(structName) == structMap.end() )
   {
      ...
      s = new MslStruct(structName, type->getLine());
