...
const char kMSLTypeNames[EgstTypeCount][32] =
{
   "void",
   "bool",
   "bvec2",
...
//const char* getMSLPrecisiontring (TPrecision p)
//{
//    switch (p) {
...
//        out << getMSLPrecisiontring (precision);
...
EMslSymbolType translateType( const TType *type )
{
   ...
   else
   {
      ...
      case EbtVoid:
         return EgstVoid;
      case EbtBool:
         return EMslSymbolType(EgstBool + (type->getRowsCount() - 1));
      case EbtInt:
         return EMslSymbolType(EgstInt + (type->getRowsCount() - 1));
      case EbtFloat:
         return EMslSymbolType(EgstFloat + (type->getRowsCount() - 1));
