...
local unzFile unzOpenInternal(const void *path, zlib_filefunc64_32_def* pzlib_filefunc64_32_def)
{
    ...
    {
        *s = us;
        MDunzGoToFirstFile((unzFile)s);
    }
    return (unzFile)s;
}

extern unzFile ZEXPORT MDunzOpen2(const char *path, zlib_filefunc_def* pzlib_filefunc32_def)
{
...
extern unzFile ZEXPORT MDunzOpen2_64(const void *path, zlib_filefunc64_def* pzlib_filefunc_def)
{
...
extern int ZEXPORT MDunzClose(unzFile file)
{
    unz64_s* s;
    if (file == NULL)
...
*/
extern int ZEXPORT MDunzOpenCurrentFile3(unzFile file, int* method, int* level, int raw, const char* password)
{
    int err = UNZ_OK;
...
extern int ZEXPORT MDunzOpenCurrentFile(unzFile file)
{
    return MDunzOpenCurrentFile3(file, NULL, NULL, 0, NULL);
}

extern int ZEXPORT MDunzOpenCurrentFilePassword(unzFile file, const char* password)
{
    return MDunzOpenCurrentFile3(file, NULL, NULL, 0, password);
}

extern int ZEXPORT MDunzOpenCurrentFile2(unzFile file, int* method, int* level, int raw)
{
    return MDunzOpenCurrentFile3(file, method, level, raw, NULL);
}
...
extern int ZEXPORT MDunzReadCurrentFile(unzFile file, voidp buf, unsigned len)
{
    int err = UNZ_OK;
    uInt read = 0;
    unz64_s* s;
...
extern int ZEXPORT MDMDunzCloseCurrentFile(unzFile file)
{
    int err = UNZ_OK;
...
extern int ZEXPORT MDunzGoToFirstFile2(unzFile file, unz_file_info64 *pfile_info, char *filename,
    uLong filename_size, void *extrafield, uLong extrafield_size, char *comment, uLong comment_size)
{
    int err = UNZ_OK;
    unz64_s* s;
...
extern int ZEXPORT MDunzGoToFirstFile(unzFile file)
{
    return MDunzGoToFirstFile2(file, NULL, NULL, 0, NULL, 0, NULL, 0);
}
...
extern int ZEXPORT unzLocateFile(unzFile file, const char *filename, unzFileNameComparer filename_compare_func)
{
    ...
    err = MDunzGoToFirstFile2(file, NULL, current_filename, sizeof(current_filename)-1, NULL, 0, NULL, 0);
