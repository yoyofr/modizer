...
{
    int err = MDunzGoToFirstFile(uf);
    if (err != UNZ_OK)
    {
        printf("error %d with zipfile in MDunzGoToFirstFile\n", err);
        return 1;
    }
...
int do_extract_currentfile(unzFile uf, int opt_extract_without_path, int* popt_overwrite, const char *password)
{
    ...
    err = MDunzOpenCurrentFilePassword(uf, password);
    if (err != UNZ_OK)
        printf("error %d with zipfile in MDunzOpenCurrentFilePassword\n", err);
    ...
    if (fout != NULL)
    {
        ...
        do
        {
            err = MDunzReadCurrentFile(uf, buf, size_buf);
            if (err < 0)
            {
                printf("error %d with zipfile in MDunzReadCurrentFile\n", err);
                break;
            }
            if (err == 0)
                break;
    ...
    errclose = MDMDunzCloseCurrentFile(uf);
    if (errclose != UNZ_OK)
        printf("error %d with zipfile in MDMDunzCloseCurrentFile\n", errclose);
...
int main(int argc, const char *argv[])
{
    ...
    if (zipfilename != NULL)
    {
        ...
        uf = MDunzOpen2_64(zipfilename, &ffunc);
#else
