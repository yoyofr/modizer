...
INT_PTR nseel_setVar(compileContext *ctx, INT_PTR varNum)
{
...

  {
    ...
    wb=((int)varNum)/NSEEL_VARS_PER_BLOCK;
    ti=(((int)varNum)%NSEEL_VARS_PER_BLOCK);
    nameptr=ctx->varTable_Names[wb]+ti*NSEEL_MAX_VARIABLE_NAMELEN;
    if (!nameptr[0]) 
    {
...
INT_PTR nseel_translate(compileContext *ctx, int type)
{
	...
	*ctx->yytext = 0;
	...
	case INTCONST:
        {
#if 1
            long long v = atoll(ctx->yytext);
//            printf("INTCONST %f\n", (EEL_F)v);
            return nseel_createCompiledValue(ctx,(EEL_F)v, NULL);
#else
            return nseel_createCompiledValue(ctx,(EEL_F)atoi(ctx->yytext), NULL);
#endif
        }
