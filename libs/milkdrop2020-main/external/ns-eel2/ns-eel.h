...
int NSEEL_init(void); // returns 0 on success. clears any added functions as well
...
int *NSEEL_getstats(void); // returns a pointer to 5 ints... source bytes, static code bytes, call code bytes, data bytes, number of code handles
EEL_F *NSEEL_getglobalregs(void);

typedef void *NSEEL_VMCTX;
typedef void *NSEEL_CODEHANDLE;

NSEEL_VMCTX NSEEL_VM_alloc(void); // return a handle
