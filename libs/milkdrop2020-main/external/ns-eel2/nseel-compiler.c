...
const static unsigned int GLUE_FUNC_ENTER[0] = {};
const static unsigned int GLUE_FUNC_LEAVE[0] = {};
...
#define newBlock(x,a) (unsigned char *)__newBlock_align(ctx,x,a)
...
INT_PTR nseel_createCompiledFunction3(compileContext *ctx, int fntype, INT_PTR fn, INT_PTR code1, INT_PTR code2, INT_PTR code3)
{
  ...
  else
  {
    ...
    myfunc = nseel_getFunctionAddress(fntype, (int)fn, &size2, &preProc,&repl);
...
NSEEL_CODEHANDLE NSEEL_code_compile(NSEEL_VMCTX _ctx, char *_expression, int lineoffs)
{
  ...
  while (*expression)
  {
    ...
    if (ctx->computTableTop > NSEEL_MAX_TEMPSPACE_ENTRIES- /* safety */ 16 - /* alignment */4 ||
        !startptr) 
    { 
      int byteoffs = (int)(expr - expression_start);
  ...
  else 
  {
    ...
    handle->blocks = (llBlock *)ctx->blocks_head;
    ctx->blocks_head=0;
