			...
			if (queued > 900 || audiosize > queued * 2)
			{
				...
				else
				{
					...
					int beststart = 0, extraAtEnd = 0;
					{
						...
						{
							int diff = ABS(this->sampleQueue[i].l - this->sampleQueue[i + 1].l) + ABS(this->sampleQueue[i].r - this->sampleQueue[i + 1].r);
					...
					while (true)
					{
						int a = ABS(this->pingpong(midpointX - midpointXOffset, queued) - midpointY) - midpointXOffset;
