...
/* removing global variables for supporting SymbianOS */
/* 051206 Daisuke Nagano <breeze.nagano@nifty.com> */
...
#else
static const UINT32 d1l_tab[16] = {
...
#else
# define PSG (_SELF->i_PSG)
# define chanout (_SELF->i_chanout)
# define m2 (_SELF->i_m2)
# define c1 (_SELF->i_c1)
# define c2 (_SELF->i_c2)
# define mem (_SELF->i_mem)
#endif


#ifdef NOGLOBAL
typedef struct _ym2151_c_instances {
  YM2151* i_PSG;
  signed int i_chanout[8];
  signed int i_m2, i_c1, i_c2;
  signed int i_mem;
} ym2151_c_instances;

static ym2151_c_instances*
__get_instances(void)
{
  return (ym2151_c_instances*)_get_ym2151_c();
}

# define _SELF (__get_instances())

void*
_ym2151_c_initialize(void)
{
  ym2151_c_instances* self = NULL;

  self = (ym2151_c_instances *)malloc(sizeof(ym2151_c_instances));
  if (!self) {
    return NULL;
  }

  memset(self, 0, sizeof(ym2151_c_instances));

  return self;
}

void
_ym2151_c_finalize(void* in_self)
{
  ym2151_c_instances* self = (ym2151_c_instances *)in_self;
  if (self) {
    free(self);
  }
}
...
{
	ym2151_c_instances* self = (ym2151_c_instances *)_chip;
	YM2151 *chip = self->i_PSG;
...
{
	ym2151_c_instances* self = __get_instances();
	YM2151 *psg;
...
void YM2151ResetChip(void *_chip)
{
	...
	YM2151WriteReg(_chip, 0x1b, 0);	/* only because of CT1, CT2 output pins */
	YM2151WriteReg(_chip, 0x18, 0);	/* set LFO frequency */
	for (i=0x20; i<0x100; i++)		/* set the operators */
	{
		YM2151WriteReg(_chip, i, 0);
	}
}
...
INLINE void chan_calc(ym2151_c_instances* self, unsigned int chan)
{
	YM2151Operator *op;
	unsigned int env;
	UINT32 AM = 0;
	YM2151* psg = self->i_PSG;

	self->i_m2 = self->i_c1 = self->i_c2 = self->i_mem = 0;
	...
	*op->mem_connect = op->mem_value;	/* restore delayed sample (MEM) value to m2 or c2 */
	...
	env = volume_calc(op);
	{
		...
		if (!op->connect)
			/* algorithm 5 */
			self->i_mem = self->i_c1 = self->i_c2 = op->fb_out_prev;
		else
			/* other algorithms */
	...
	if (env < ENV_QUIET)
		*(op+1)->connect += op_calc(op+1, env, self->i_m2);

	env = volume_calc(op+2);	/* C1 */
	if (env < ENV_QUIET)
		*(op+2)->connect += op_calc(op+2, env, self->i_c1);

	env = volume_calc(op+3);	/* C2 */
	if (env < ENV_QUIET)
		self->i_chanout[chan]    += op_calc(op+3, env, self->i_c2);

	/* M1 */
	op->mem_value = self->i_mem;
}
INLINE void chan7_calc(ym2151_c_instances* self)
{
	YM2151Operator *op;
	unsigned int env;
	UINT32 AM = 0;
	...
	*op->mem_connect = op->mem_value;	/* restore delayed sample (MEM) value to m2 or c2 */
	...
	if (psg->noise & 0x80)
	{
		...
		self->i_chanout[7] += ((psg->noise_rng&0x10000) ? noiseout: -noiseout); /* bit 16 -> output */
	}
	else
	{
		if (env < ENV_QUIET)
			self->i_chanout[7] += op_calc(op+3, env, self->i_c2);
	}
	/* M1 */
	op->mem_value = self->i_mem;
}
...
INLINE void advance_eg(ym2151_c_instances* self)
{
	YM2151Operator *op;
	unsigned int i;
	YM2151* psg = self->i_PSG;
...
INLINE void advance(ym2151_c_instances* self)
{
	YM2151Operator *op;
	unsigned int i;
	int a,p;
...
void YM2151UpdateOne(void *chip, SAMP **buffers, int length)
{
	...
	SAMP *bufL, *bufR;
	ym2151_c_instances* self = (ym2151_c_instances *)chip;
	YM2151* psg = self->i_PSG;
	...
	for (i=0; i<length; i++)
	{
		...
		outl = self->i_chanout[0] & psg->pan[0];
		outr = self->i_chanout[0] & psg->pan[1];
		outl += (self->i_chanout[1] & psg->pan[2]);
		outr += (self->i_chanout[1] & psg->pan[3]);
		outl += (self->i_chanout[2] & psg->pan[4]);
		outr += (self->i_chanout[2] & psg->pan[5]);
		outl += (self->i_chanout[3] & psg->pan[6]);
		outr += (self->i_chanout[3] & psg->pan[7]);
		outl += (self->i_chanout[4] & psg->pan[8]);
		outr += (self->i_chanout[4] & psg->pan[9]);
		outl += (self->i_chanout[5] & psg->pan[10]);
		outr += (self->i_chanout[5] & psg->pan[11]);
		outl += (self->i_chanout[6] & psg->pan[12]);
		outr += (self->i_chanout[6] & psg->pan[13]);
		outl += (self->i_chanout[7] & psg->pan[14]);
		outr += (self->i_chanout[7] & psg->pan[15]);
...
{
	ym2151_c_instances* self = (ym2151_c_instances *)chip;
	YM2151 *localPSG = self->i_PSG;
	localPSG->irqhandler = handler;
}
