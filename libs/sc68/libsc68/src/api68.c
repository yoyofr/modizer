...
static u8 trap_func[] = {
#if defined(EMSCRIPTEN)||defined(__APPLE__)
// just skip the silly assembling with that garbage toolchain
96,16,8,56,0,3,250,23,103,6,17,252,0,223,250,17,78,115,72,231,128,224,49,252,0,16,4,26,65,250,2,76,33,200,5,160,65,250,0,86,33,200,0,20,33,200,0,128,65,250,0,76,33,200,0,132,65,250,0,218,33,200,0,184,67,248,250,0,66,41,0,29,65,250,255,184,33,200,1,20,48,60,0,32,1,137,0,7,1,137,0,19,17,252,0,192,250,35,17,252,0,80,250,29,65,250,0,142,67,239,252,0,33,9,76,223,7,1,78,117,76,223,127,255,78,115,72,231,255,254,77,239,0,66,48,30,176,124,0,9,103,34,176,124,0,32,103,46,176,124,0,48,103,78,176,124,0,72,103,88,176,124,0,73,103,102,78,114,47,33,78,112,96,0,255,202,32,94,112,0,82,64,103,4,74,24,102,248,46,128,96,0,255,184,32,30,103,26,176,188,0,0,0,1,103,4,112,0,103,16,64,192,2,64,32,0,86,192,2,64,0,1,103,2,112,1,46,128,96,0,255,146,32,60,0,0,21,0,46,128,96,0,255,134,0,0,0,0,65,250,255,254,32,32,144,158,8,128,0,0,32,128,46,128,96,0,255,110,112,0,46,128,96,0,255,102,72,231,255,254,77,239,0,66,48,30,176,124,0,31,103,0,0,226,176,124,0,32,103,84,176,124,0,38,103,22,176,124,0,128,103,26,176,124,0,129,103,40,78,114,47,46,78,112,96,0,255,50,72,122,255,46,47,30,78,117,0,0,48,60,255,127,65,250,255,250,74,224,102,2,112,1,46,128,96,0,255,20,112,128,65,250,255,232,74,32,103,4,66,16,112,0,46,128,96,0,255,0,0,0,0,0,0,0,65,250,255,254,34,94,179,252,0,0,0,1,102,4,46,160,96,80,69,248,136,0,112,0,50,40,255,252,16,25,107,10,225,72,16,25,1,138,0,0,96,242,176,60,0,128,102,4,18,25,96,232,176,60,0,129,102,24,16,25,20,25,22,25,24,0,225,76,24,1,9,138,0,0,182,1,103,16,146,2,96,238,16,25,103,8,78,114,47,46,78,112,96,4,33,9,49,1,96,0,254,148,1,52,250,25,250,31,240,5,1,32,250,27,250,33,240,0,1,20,250,29,250,35,15,21,1,16,250,29,250,37,240,20,69,250,255,222,112,3,192,94,231,72,212,192,58,106,0,2,24,21,16,42,0,6,200,0,26,132,50,30,70,0,91,194,2,66,0,4,229,105,194,64,136,65,56,106,0,4,48,30,24,128,112,7,114,0,18,42,0,7,192,1,230,73,67,248,250,0,1,241,16,7,1,241,16,19,52,82,36,158,26,132,96,0,254,32,95,77,67,72,0,1,0,0,95,67,80,85,0,0,0,0,83,67,54,56,48,0,2,183,0,0,0,0
#else
...
#if defined(EMSCRIPTEN)||defined(__APPLE__)
	// keep it simple and mirror "buffer" handling
    short*	scopes[8];	// hack: what is the max number?
#endif
...
extern int getNumberTraceStreams(); // silly that sc68's internal APIs don't work for thia

// hack; use global vars to avoid changing sc68 APIs	
s32		 *gMixBuffer;
short **gIntScopeBufs;

static void clear_scopes(sc68_t * sc68) {
	int bufreq = sc68->mix.bufreq;
	
	int track  = sc68_cntl(sc68,SC68_GET_TRACK);			
	int n = sc68_used_channels(sc68, track);
	
	for (int i= 0; i<n; i++) {	// only the used ones
		if (gIntScopeBufs[i]) {
			memset(gIntScopeBufs[i], 0, bufreq << 2);
		}
		else {
			fprintf(stderr, "error: gIntScopeBufs %d is NULL\n", i);
		}
	}
}

static void destroy_scopes(sc68_t * sc68) {
	for (int i= 0; i<8; i++) {
		if(sc68->mix.scopes[i]) free(sc68->mix.scopes[i]);
		sc68->mix.scopes[i] = NULL;
	}
}

static void alloc_scopes(sc68_t * sc68) {
	int len = sc68->mix.bufreq << 2;

	for (int i= 0; i<8; i++) {
		sc68->mix.scopes[i] = malloc(len);
	}
}


extern short **getScopeBuffers();

int sc68_mix_buf_len(sc68_t *sc68) {
	return sc68->mix.bufreq;
}
#endif
...
#if defined(EMSCRIPTEN)||defined(__APPLE__)
	// what dumnfuckery to use 1 as a default initializer
	for (int i= 0; i<8; i++) {
		sc68->mix.scopes[i] = NULL;
	}
	gIntScopeBufs = &(sc68->mix.scopes[0]);
...
#if defined(EMSCRIPTEN)||defined(__APPLE__)
unsigned char sc68_used_hw(sc68_t * sc68, int track)
{
  const disk68_t  *d = sc68->disk;
  const music68_t *m = d->mus + track - 1;
  return m->hwflags & 7;
}

/* Hardware table */
static const unsigned char chnTable[8] = {
	0, //"none"											unused
	3, //"Yamaha-2149"
	4, // "MicroWire (STE)"  							just leave the 3 Yamaha-2149 voices empty
	4, // "Yamaha-2149 & MicroWire (STE)"				
	4, // "Amiga/Paula"
	0, // "Yamaha-2149 & Amiga/Paula"					unused
	0, // "MicroWire (STE) & Amiga/Paula"				unused
	0, // "Yamaha-2149 & MicroWire (STE) & Amiga/Paula"	unused
};

const char* sc68_used_hw_label(sc68_t * sc68, int track)
{
	return hwtable[sc68_used_hw(sc68, track)];
}

unsigned char sc68_used_channels(sc68_t * sc68, int track)
{
	return chnTable[sc68_used_hw(sc68, track)];
}
...
#if defined(EMSCRIPTEN)||defined(__APPLE__)
		gMixBuffer = (s32*)sc68->mix.buffer;	// used to position scope offset
...
#if defined(EMSCRIPTEN)||defined(__APPLE__)
		// copy the scope data using the same offsets used for the main sample data
		
		int num =  getNumberTraceStreams(); // sc68_used_channels(sc68, track);
		short **sb= getScopeBuffers();
		
        if (sb) {
            for(int i= 0; i<num; i++) {
                if(sc68->mix.scopes[i])  memcpy(&(sb[i][scopeOffset]), &(sc68->mix.scopes[i][sc68->mix.bufpos]), len * sizeof(short));
            }
        }
		scopeOffset += len;
#endif		  	  
