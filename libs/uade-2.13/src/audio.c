...
static inline void sample_backend(int left1,int left2, int right1,int right2) //0,3 1,2
{
    ...
    for (;;) {
        m_voice_buff[0][(ofs_start>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)&(SOUND_BUFFER_SIZE_SAMPLE*4*4-1)]=LIMIT8((left1>>6)*0.7);
        m_voice_buff[3][(ofs_start>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)&(SOUND_BUFFER_SIZE_SAMPLE*4*4-1)]=LIMIT8((left2>>6)*0.7);
        m_voice_buff[1][(ofs_start>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)&(SOUND_BUFFER_SIZE_SAMPLE*4*4-1)]=LIMIT8((right1>>6)*0.7);
        m_voice_buff[2][(ofs_start>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)&(SOUND_BUFFER_SIZE_SAMPLE*4*4-1)]=LIMIT8((right2>>6)*0.7);
    ...
    for (int jj=0;jj<4;jj++) {
        ...
        if ((m_voice_current_ptr[jj]>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)>=SOUND_BUFFER_SIZE_SAMPLE*4*4) m_voice_current_ptr[jj]-=(SOUND_BUFFER_SIZE_SAMPLE*4*4)<<MODIZER_OSCILLO_OFFSET_FIXEDPOINT;
    }
...
static void sample16si_sinc_handler (void)
{
    ...
    for (;;) {
        m_voice_buff[0][(ofs_start>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)&(SOUND_BUFFER_SIZE_SAMPLE*4*4-1)]=LIMIT8((datas[0]>>6)*0.7);
        m_voice_buff[3][(ofs_start>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)&(SOUND_BUFFER_SIZE_SAMPLE*4*4-1)]=LIMIT8((datas[3]>>6)*0.7);
        m_voice_buff[1][(ofs_start>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)&(SOUND_BUFFER_SIZE_SAMPLE*4*4-1)]=LIMIT8((datas[1]>>6)*0.7);
        m_voice_buff[2][(ofs_start>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)&(SOUND_BUFFER_SIZE_SAMPLE*4*4-1)]=LIMIT8((datas[2]>>6)*0.7);
        ofs_start+=1<<MODIZER_OSCILLO_OFFSET_FIXEDPOINT;
        if (ofs_start>=ofs_end) break;
    }
