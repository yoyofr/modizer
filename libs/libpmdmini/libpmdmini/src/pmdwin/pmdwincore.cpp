...
void WINAPI PMDWIN::getpcmdata(int16_t *buf, int32_t nsamples)
{
	...
	do {
		if(nsamples - copysamples <= us2) {
            ...
            //YOYOFR
            m_voice_current_system=pmd_system_voice_idx[VOICE_PPZ];
            m_voice_current_samplerate=open_work.ppzrate;
            //YOYOFR
            ...
            if (m_voice_current_system>=0) {
                for (int i=0;i<8;i++) {
                    int64_t ofs_end=m_voice_current_ptr[m_voice_current_system+i]+((int64_t)(us2)<<MODIZER_OSCILLO_OFFSET_FIXEDPOINT);
                    m_voice_current_ptr[m_voice_current_system+i]=ofs_end;
                }
            }
            //YOYOFR
