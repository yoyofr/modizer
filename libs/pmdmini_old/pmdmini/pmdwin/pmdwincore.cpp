...
bool WINAPI PMDWIN::getlength(TCHAR *filename, int *length, int *loop)
{
	...
	*length = 0;
    ...
    if (fmUsed) {
        pmd_real_tracks_used+=NumOfFMPart;
        printf("FM ON\n");
    }
    if (ssgUsed) {
        pmd_real_tracks_used+=NumOfSSGPart;
        printf("SSG ON\n");
    }
    if (adpUsed) {
        pmd_real_tracks_used+=NumOfADPCMPart;
        printf("ADPCM ON\n");
    }
    if (rhyUsed) {
    ...
    if (extUsed) {
        pmd_real_tracks_used+=NumOfExtPart;
        printf("EXT ON\n");
    }
    //if (dumUsed) pmd_real_tracks_used++;
    if (effUsed) {
        pmd_real_tracks_used+=NumOfEffPart;
        printf("Eff ON\n");
    }
    if (ppzUsed) {
        pmd_real_tracks_used+=NumOfPPZ8Part;
        printf("PPZ8 ON\n");
    }
