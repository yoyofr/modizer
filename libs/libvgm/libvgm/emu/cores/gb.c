...
static void gameboy_update(void *chip, UINT32 samples, DEV_SMPL **outputs)
{
    ...
    for (i = 0; i < samples; i++)
    {
        ...
        if (gb->snd_1.on && !gb->snd_1.Muted)
        {
            ...
            //YOYOFR
            if (gb->snd_control.mode1_left || gb->snd_control.mode1_right) chanout[0]=sample;
        }
        ...
        if (gb->snd_2.on && !gb->snd_2.Muted)
        {
            ...
            //YOYOFR
            if (gb->snd_control.mode2_left || gb->snd_control.mode2_right) chanout[1]=sample;
        }
        ...
        if (gb->snd_3.on && !gb->snd_3.Muted)
        {
            ...
            //YOYOFR
            if (gb->snd_control.mode3_left || gb->snd_control.mode3_right) chanout[2]=sample;
        }
        ...
        if (gb->snd_4.on && !gb->snd_4.Muted)
        {
            ...
            //YOYOFR
            if (gb->snd_control.mode4_left || gb->snd_control.mode4_right) chanout[3]=sample;
        }
        ...
        if (m_voice_ofs>=0)
            for (int jj=0;jj<4;jj++) {
                ...
                if (ofs_end>ofs_start)
                    for (;;) {
                        int val=chanout[jj];
                        m_voice_buff[m_voice_ofs+jj][(ofs_start>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)&(SOUND_BUFFER_SIZE_SAMPLE*4*2-1)]=LIMIT8( val>>0 );
                ...
                if (i==0) {
                    ...
                    int vol=0;
                    switch (jj) {
                        case 0:muteOff=(gb->snd_1.on && !gb->snd_1.Muted);
                            freq=gb->snd_1.frequency;
                            vol=gb->snd_1.envelope_value;
                            break;
                        case 1:muteOff=(gb->snd_2.on && !gb->snd_2.Muted);
                            freq=gb->snd_2.frequency;
                            vol=gb->snd_2.envelope_value;
                            break;
                        case 2:muteOff=(gb->snd_3.on && !gb->snd_3.Muted);
                            freq=gb->snd_3.frequency;
                            vol=1;
                            break;
                        case 3:muteOff=(gb->snd_4.on && !gb->snd_4.Muted);
                            freq=gb->snd_4.frequency;
                            vol=gb->snd_4.envelope_value;
                            break;
                    }
                    if (muteOff) {
                        if (freq && vol) {
