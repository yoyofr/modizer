...
static void k051649_update(void *param, UINT32 samples, DEV_SMPL **outputs)
{
    ...
    for (j = 0; j < 5; j++)
    {
        ...
        if (voice[j].frequency > 8 && ! voice[j].Muted)
        {
            ...
            //YOYOFR
            if (voice[j].key) {
                int vol=voice[j].volume;
                if (vol) {
                    double freq=(double)(voice[j].frequency);
                    if (freq) {
                        ...
                        if (voice[j].frequency > 8 && ! voice[j].Muted) {
                            if (ofs_end>ofs_start)
                                for (;;) {
                                    int val=0;
                                    val=smpl*256/8;
                                    m_voice_buff[m_voice_ofs+j][(ofs_start>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)&(SOUND_BUFFER_SIZE_SAMPLE*4*2-1)]=LIMIT8( val>>6 );
            ...
            if (m_voice_ofs>=0) {
                int64_t ofs_start=m_voice_current_ptr[m_voice_ofs+j];
                int64_t ofs_end=(m_voice_current_ptr[m_voice_ofs+j]+samples*smplIncr);
                    while ((ofs_end>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)>=SOUND_BUFFER_SIZE_SAMPLE*4*2) ofs_end-=(SOUND_BUFFER_SIZE_SAMPLE*4*2<<MODIZER_OSCILLO_OFFSET_FIXEDPOINT);
                    m_voice_current_ptr[m_voice_ofs+j]=ofs_end;
            }
            //TODO:  MODIZER changes end / YOYOFR
