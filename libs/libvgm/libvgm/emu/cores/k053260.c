    ...
    if ((info->mode & 2) && info->rom != NULL)
    {
        ...
        for (j = 0; j < samples; j++)
        {
            ...
            for ( i = 0; i < 4; i++ )
            {
                    ...
                    if (info->voice[i].playing && !info->voice[i].Muted) {
                        if (ofs_end>ofs_start)
                            for (;;) {
                                ...
                                val=(info->voice[i].output * (info->voice[i].pan_volume[0]+info->voice[i].pan_volume[1])) >> 15;
                                
                                m_voice_buff[m_voice_ofs+i][(ofs_start>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)&(SOUND_BUFFER_SIZE_SAMPLE*4*2-1)]=LIMIT8( val>>8 );
                ...
                //YOYOFR
                if (info->voice[i].playing && !info->voice[i].Muted) {
                    int vol=info->voice[i].volume;
                    if (vol) {
                        double freq=(double)(info->voice[i].pitch);
                        if (freq) {
                            freq=(double)(info->clock)/(4096-freq);
