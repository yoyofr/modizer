...
int tim_loop_max,tim_loop_cnt;
int tim_loop_point;
static void skip_to(int32 until_time);
extern int tim_midilength;
...
#define EVENT_NAME(X) case X: return #X
    switch (type) {
            ...
            EVENT_NAME(ME_STEP);
            //YOYOFR
            EVENT_NAME(ME_LOOP_START);
            EVENT_NAME(ME_LOOP_END);
            //YOYOFR
            EVENT_NAME(ME_LAST);
            EVENT_NAME(ME_EOT);
    }
                              ...
                              int nsp, int *note, int *vlist, MidiEvent *e)
{
    ...
    for (i = 0, sp = splist; i < nsp; i++, sp++) {
        ...
        if (sp->low_freq <= fst && sp->high_freq >= fst
            ...
            && ! (sp->inst_type == INST_SF2
                  && sp->tim_sample_type == SF_SAMPLETYPE_RIGHT)) {
    ...
    for (i = 0; i < nvc; i++) {
        ...
        if (spc->inst_type == INST_SF2
            && spc->tim_sample_type == SF_SAMPLETYPE_LEFT) {
...
static void play_midi_prescan(MidiEvent *ev) {
    ...
    tim_loop_cnt=0;//YOYOFR
    int tim_loop_time=0;//YOYOFR
    ...
    tim_midilength=(int)((float)(ev->time + tim_loop_time)*1000/(float)(play_mode->rate));
    //YOYOFR
    /*    printf("max ch: %d\n",ch_idx); //YOYOFR
     for (int ii=0;ii<ch_idx;ii++) {
     printf("%d ch %d\n",ii,ch_used[ii]);
    ...
    do
    {
        switch(rc=ctl->read(&val))
        {
            ...
            case RC_JUMP:
                ...
                tim_loop_cnt=0;
                if (tim_loop_cnt<tim_loop_max) {
                    while (val >= sample_count) {
                        val-=sample_count-tim_loop_point;
                        tim_loop_cnt++;
                        if (tim_loop_cnt==tim_loop_max) break;
                    }
                }
...
{
    free(reverb_buffer);reverb_buffer=NULL;
}
