...
#define CACHE_DATA_LEN (allocate_cache_size / sizeof(tim_sample_t))
...
static tim_sample_t *cache_data = NULL;
...
static void loop_connect(tim_sample_t *, int32, int32);

void free_cache_data(void) {
	free(cache_data);cache_data=NULL;
...
{
	if (cache_data == NULL) {
		cache_data = (tim_sample_t *)
				safe_large_malloc((CACHE_DATA_LEN + 1) * sizeof(tim_sample_t));
		memset(cache_data, 0, (CACHE_DATA_LEN + 1) * sizeof(tim_sample_t));
...
static void loop_connect(tim_sample_t *data, int32 start, int32 end)
{
	int i, mixlen;
	int32 t0, t1;
