...
unsigned short sexySPUreadRegister(unsigned long reg)
{
 const unsigned long r=reg&0xfff;
...
static void SoundOn(int start,int end,unsigned short val)     // SOUND ON PSX COMAND
{
 int ch;
   ...
   if((val&1) && s_chan[ch].pStart)                    // mmm... start has to be set before key on !?!
    {
     ...
     s_chan[ch].bNew=1;
     //TODO - dwNewChannel|=(1<<ch);                            // bitfield for faster testing
    }
  }
}
...
static void SoundOff(int start,int end,unsigned short val)    // SOUND OFF PSX COMMAND
{
 int ch;
 for(ch=start;ch<end;ch++,val>>=1)                     // loop channels
  {
...
static void FModOn(int start,int end,unsigned short val)      // FMOD ON PSX COMMAND
{
 int ch;
...
static void NoiseOn(int start,int end,unsigned short val)     // NOISE ON PSX COMMAND
{
 int ch;
 ...
 if(vol&0x8000)                                        // sweep?
  {
   ...
   vol=((vol&0x7f)+1)>>1;							   // /2; // -> sweep: 0..127 -> 0..64
...
static void ReverbOn(int start,int end,unsigned short val)    // REVERB ON PSX COMMAND
{
 int ch;
