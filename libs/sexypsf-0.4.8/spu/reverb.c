...
INLINE void StoreREVERB(SPUCHAN * pChannel)
{
 if(iUseReverb==0) return;
 else
 if(iUseReverb==2) // -------------------------------- // Neil's reverb
  {
   const int iRxl=(pChannel->sval*pChannel->iLeftVolume)>>14;	// /0x4000;
   const int iRxr=(pChannel->sval*pChannel->iRightVolume)>>14;  // /0x4000;
 ...
 else // --------------------------------------------- // Pete's easy fake reverb
  {
   ...
   int iRxl=(pChannel->sval*pChannel->iLeftVolume)>>15;	// /0x8000;
   int iRxr=(pChannel->sval*pChannel->iRightVolume)>>15; // /0x8000;

   for(iRn=1;iRn<=pChannel->iRVBNum;iRn++,iRr+=pChannel->iRVBRepeat,iRxl>>=1,iRxr>>=1)
    {
     pN=sRVBPlay+((pChannel->iRVBOffset+iRr)<<1);
   ...
   if(iCnt&1)                                          // we work on every second left value: downsample to 22 khz
    {
     if(spuCtrl&0x80)                                  // -> reverb on? oki
      {
       register int ACC0,ACC1,FB_A0,FB_A1,FB_B0,FB_B1;

       const int INPUT_SAMPLE_L=*(sRVBStart);
       const int INPUT_SAMPLE_R=*(sRVBStart+1);

       /* TODO - Is >> really faster than / ? ...
	   ...
	   */
	   ...
	   const int IIR_INPUT_A0 = ((g_buffer(rvb.IIR_SRC_A0) * rvb.IIR_COEF)>>15L) + ((INPUT_SAMPLE_L * rvb.IN_COEF_L)>>15L);
       const int IIR_INPUT_A1 = ((g_buffer(rvb.IIR_SRC_A1) * rvb.IIR_COEF)>>15L) + ((INPUT_SAMPLE_R * rvb.IN_COEF_R)>>15L);
       const int IIR_INPUT_B0 = ((g_buffer(rvb.IIR_SRC_B0) * rvb.IIR_COEF)>>15L) + ((INPUT_SAMPLE_L * rvb.IN_COEF_L)>>15L);
       const int IIR_INPUT_B1 = ((g_buffer(rvb.IIR_SRC_B1) * rvb.IIR_COEF)>>15L) + ((INPUT_SAMPLE_R * rvb.IN_COEF_R)>>15L);

       const int IIR_A0 = ((IIR_INPUT_A0 * rvb.IIR_ALPHA)>>15L) + ((g_buffer(rvb.IIR_DEST_A0) * (32768L - rvb.IIR_ALPHA))>>15L);
       const int IIR_A1 = ((IIR_INPUT_A1 * rvb.IIR_ALPHA)>>15L) + ((g_buffer(rvb.IIR_DEST_A1) * (32768L - rvb.IIR_ALPHA))>>15L);
       const int IIR_B0 = ((IIR_INPUT_B0 * rvb.IIR_ALPHA)>>15L) + ((g_buffer(rvb.IIR_DEST_B0) * (32768L - rvb.IIR_ALPHA))>>15L);
       const int IIR_B1 = ((IIR_INPUT_B1 * rvb.IIR_ALPHA)>>15L) + ((g_buffer(rvb.IIR_DEST_B1) * (32768L - rvb.IIR_ALPHA))>>15L);
	   ...
	   */
	   ...
	   ACC0 = ((g_buffer(rvb.ACC_SRC_A0) * rvb.ACC_COEF_A)>>15L) +
              ((g_buffer(rvb.ACC_SRC_B0) * rvb.ACC_COEF_B)>>15L) +
              ((g_buffer(rvb.ACC_SRC_C0) * rvb.ACC_COEF_C)>>15L) +
              ((g_buffer(rvb.ACC_SRC_D0) * rvb.ACC_COEF_D)>>15L);
       ACC1 = ((g_buffer(rvb.ACC_SRC_A1) * rvb.ACC_COEF_A)>>15L) +
              ((g_buffer(rvb.ACC_SRC_B1) * rvb.ACC_COEF_B)>>15L) +
              ((g_buffer(rvb.ACC_SRC_C1) * rvb.ACC_COEF_C)>>15L) +
              ((g_buffer(rvb.ACC_SRC_D1) * rvb.ACC_COEF_D)>>15L);
	   ...
	   */
	   ...
	   s_buffer(rvb.MIX_DEST_A0, ACC0 - ((FB_A0 * rvb.FB_ALPHA)>>15L));
       s_buffer(rvb.MIX_DEST_A1, ACC1 - ((FB_A1 * rvb.FB_ALPHA)>>15L));

       s_buffer(rvb.MIX_DEST_B0, ((rvb.FB_ALPHA * ACC0)>>15L) - (FB_A0 * (int)((rvb.FB_ALPHA^0xFFFF8000))>>15L) - ((FB_B0 * rvb.FB_X)>>15L));
       s_buffer(rvb.MIX_DEST_B1, ((rvb.FB_ALPHA * ACC1)>>15L) - (FB_A1 * (int)((rvb.FB_ALPHA^0xFFFF8000))>>15L) - ((FB_B1 * rvb.FB_X)>>15L));

       rvb.iLastRVBLeft  = rvb.iRVBLeft;
       rvb.iLastRVBRight = rvb.iRVBRight;

       rvb.iRVBLeft  = (g_buffer(rvb.MIX_DEST_A0)+g_buffer(rvb.MIX_DEST_B0))>>1; 	// /3;
       rvb.iRVBRight = (g_buffer(rvb.MIX_DEST_A1)+g_buffer(rvb.MIX_DEST_B1))>>1;	// /3;

       rvb.iRVBLeft  = (rvb.iRVBLeft  * rvb.VolLeft) >> 14;		// / 0x4000;
       rvb.iRVBRight = (rvb.iRVBRight * rvb.VolRight) >> 14;	// / 0x4000;

       rvb.CurrAddr++;
       if(rvb.CurrAddr>0x3ffff) rvb.CurrAddr=rvb.StartAddr;

       return rvb.iLastRVBLeft + ((rvb.iRVBLeft-rvb.iLastRVBLeft)>>1);	// /2;
      }
     else                                              // -> reverb off
      {
       rvb.iLastRVBLeft=rvb.iLastRVBRight=rvb.iRVBLeft=rvb.iRVBRight=0;
...
INLINE int MixREVERBRight(void)
{
 ...
 if(iUseReverb==2)                                     // Neill's reverb:
  {
   int i=rvb.iLastRVBRight + ((rvb.iRVBRight-rvb.iLastRVBRight)>>1); // /2;
   rvb.iLastRVBRight=rvb.iRVBRight;
