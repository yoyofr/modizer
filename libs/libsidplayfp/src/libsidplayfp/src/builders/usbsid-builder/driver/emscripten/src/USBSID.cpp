...
USBSID_Class::USBSID_Class() :
  ...
  us_Found(0)
{
  USBDBG("[USBSID] Driver init start\n");
...
int USBSID_Class::USBSID_Init(bool start_threaded, bool with_cycles)
{
  ...
  if (!us_PortIsOpen) {
    ...
    if (rc >= 0) {
      ...
      us_PortIsOpen = true;
      USBDBG("[USBSID] Opened\n");
      return rc;
    } else {
      USBDBG("[USBSID] Not found\n");
      return -1;
    }
  } else {
    USBDBG("[USBSID] Driver already started\n");
    return 0;
  }
}
...
int USBSID_Class::USBSID_Close(void)
{
  ...
  if (rc != -1) USBERR("Expected rc == -1, received: %d\n", rc);
  if (e != 0) USBERR("Expected e == 0, received: %d\n", e);
  if (devh != NULL) USBERR("Expected dev == NULL, received: %p", (void*)&devh);
  if (us_PortIsOpen) us_PortIsOpen = false;
  USBDBG("[USBSID] De-init finished\n");
  return 0;
}
...
{
  if (!us_Initialised) return;
  USBDBG("[USBSID] Reset All Registers\n");
...
void USBSID_Class::USBSID_SetClockRate(long clockrate_cycles, bool suspend_sids)
{
  ...
  for (uint8_t i = 0; i < 4; i++) {
    if (clockSpeed[i] == clockrate_cycles) {
      ...
      USBDBG("[USBSID] Clockspeed set to: %ld\n", cycles_per_sec);
      USBDBG("[USBSID] Cycles per raster %ld\n", cycles_per_raster);
      USBDBG("[USBSID] Cycles per frame: %ld\n", cycles_per_frame);
      USBDBG("[USBSID] CPU cycle duration in nanoseconds %f\n", us_CPUcycleDuration);
      USBDBG("[USBSID] Inverted CPU cycle duration in nanoseconds %.09f\n", us_InvCPUcycleDurationNanoSeconds);
...
void USBSID_Class::USBSID_SingleWrite(unsigned char *buff, size_t len)
{
  ...
  if (libusb_bulk_transfer(devh, EP_OUT_ADDR, buff, len, &actual_length, 0) < 0) {
    USBERR("[USBSID] Error while sending synchronous write buffer of length %d\n", actual_length);
  }
  return;
}
...
unsigned char USBSID_Class::USBSID_SingleRead(uint8_t reg)
{
  ...
  if (libusb_bulk_transfer(devh, EP_OUT_ADDR, buff, 3, &actual_length, 0) < 0) {
    USBERR("[USBSID] Error while sending write command for reading\n");
  }
  rc = libusb_bulk_transfer(devh, EP_IN_ADDR, result, 1, &actual_length, 0);
  if (rc == LIBUSB_ERROR_TIMEOUT) {
    USBERR("[USBSID] Timeout error while reading (%d)\n", actual_length);
    return 0;
  } else if (rc < 0) {
    USBERR("[USBSID] Error while waiting for char while reading\n");
    return 0;
  }
  return result[0];
}
...
void USBSID_Class::USBSID_StopThread(void)
{
  ...
  if (USBSID_IsRunning() == 1) {
    ...
    USBDBG("[USBSID] Set thread exit = 1\n");
    USBSID_DeInitRingBuffer();
    #ifndef EMSCRIPTEN
    pthread_join(us_ptid, NULL);
    USBDBG("[USBSID] Thread attached\n");
    threaded = withcycles = false;
    while (us_thread > 0) {}; /* Wait for the thread to finish */
    #else
    pthread_tryjoin_np(us_ptid, NULL);
    threaded = withcycles = false;
    #endif
...
{
  if (!us_Initialised) return;
  USBDBG("[USBSID] Restart thread (%d)\n", USBSID_IsRunning());
...
{
  if (!us_Initialised) return;
  USBDBG("[USBSID] Disable thread (%d)\n", USBSID_IsRunning());
  USBSID_StopThread();
  return;
}
...
void USBSID_Class::USBSID_WriteRing(uint8_t reg, uint8_t val)
{
  ...
  if (threaded && !withcycles) {
    ...
    USBERR("[USBSID] Function '%s' cannot be used when threaded = %d and withcycles = %d\n", __func__, threaded, withcycles);
  }
  return;
}
...
{ /* Returns the waited microseconds since last target time ~ not the actual cycles */
  ...
  // USBDBG("[C] %ld [WC] %ld [us] %ld [ns] %ld [ts] %lu\n",
...
int USBSID_Class::LIBUSB_OpenDevice(void)
{
  ...
  if (!devh) {
    rc = -1;
    USBERR("[USBSID] Error opening USB device with VID & PID: %d %s: %s\n", rc, libusb_error_name(rc), libusb_strerror(rc));
  }
  return rc;
}
  ...
  if (devh) {
    ...
    for (int if_num = 0; if_num < 2; if_num++) {
      if (libusb_kernel_driver_active(devh, if_num)) {
        USBDBG("[USBSID] Detaching kernel driver for interface: %d\n", if_num);
        rc = libusb_detach_kernel_driver(devh, if_num);
        USBERR("[USBSID] Error, in libusb_detach_kernel_driver: %d, %s: %s\n", rc, libusb_error_name(rc), libusb_strerror(rc));
      }
    #else
    int if_num = 4;
    #endif
      USBDBG("[USBSID] Releasing interface: %d\n", if_num);
...
int USBSID_Class::LIBUSB_DetachKernelDriver(void)
{
  ...
  int if_num = 4;
    ...
    USBDBG("[USBSID] Claiming interface: %d\n", if_num);
    rc = libusb_claim_interface(devh, if_num);
    if (rc < 0) {
      USBERR("[USBSID] Error claiming interface: %d, %s: %s\n", rc, libusb_error_name(rc), libusb_strerror(rc));
      rc = -1;
      #ifndef EMSCRIPTEN
      break;
      #endif
...
int NOINLINE USBSID_Class::LIBUSB_ConfigureDevice(void)
{
  ...
  if (rc < 0 || rc != 7) {  /* should return 7 for the encoding size */
    USBERR("[USBSID] Error configuring line encoding during control transfer: %d, %s: %s\n", rc, libusb_error_name(rc), libusb_strerror(rc));
    rc = -1;
    return rc;
  }
  #endif
...
void USBSID_Class::LIBUSB_InitOutBuffer(void)
{
  ...
  USBDBG("[USBSID] Alloc out_buffer complete\n");
  transfer_out = libusb_alloc_transfer(0);
  USBDBG("[USBSID] Alloc transfer_out complete\n");
  libusb_fill_bulk_transfer(transfer_out, devh, EP_OUT_ADDR, out_buffer, len_out_buffer, usb_out, NULL, 0);
  USBDBG("[USBSID] libusb_fill_bulk_transfer transfer_out complete\n");
...
void USBSID_Class::LIBUSB_FreeOutBuffer(void)
{
  ...
  if (out_buffer_dma) {
    ...
    if (rc < 0) {
      USBERR("[USBSID] Error, failed to free out_buffer DMA memory: %d, %s: %s\n", rc, libusb_error_name(rc), libusb_strerror(rc));
    }
  } else {
...
void USBSID_Class::LIBUSB_InitInBuffer(void)
{
  ...
  USBDBG("[USBSID] Alloc in_buffer complete\n");
  transfer_in = libusb_alloc_transfer(0);
  USBDBG("[USBSID] Alloc transfer_in complete\n");
...
void USBSID_Class::LIBUSB_FreeInBuffer(void)
{
  ...
  if (in_buffer_dma) {
    ...
    if (rc < 0) {
      USBERR("[USBSID] Error, failed to free in_buffer DMA memory: %d, %s: %s\n", rc, libusb_error_name(rc), libusb_strerror(rc));
    }
  } else {
...
void USBSID_Class::LIBUSB_StopTransfers(void)
{
  ...
  if (transfer_out != NULL) {
    ...
    if (rc < 0 && rc != -5) {
      USBERR("[USBSID] Error, failed to cancel transfer %d - %s: %s\n", rc, libusb_error_name(rc), libusb_strerror(rc));
    }
  }
...
int NOINLINE USBSID_Class::LIBUSB_Setup(bool start_threaded, bool with_cycles)
{
  ...
  // rc = libusb_init_context(NULL, /*options=*/NULL, /*num_options=*/0);  // NOTE: REQUIRES LIBUSB 1.0.27!!
  if (rc != 0) {
    USBERR("[USBSID] Error initializing libusb: %d %s: %s\n", rc, libusb_error_name(rc), libusb_strerror(rc));
    goto out;
  }
  ...
  if (LIBUSB_Available(ctx, VENDOR_ID, PRODUCT_ID) <= 0) {
    USBERR("[USBSID] No USBSID-Pico found, is the device plugged in?\n");
    goto out;
  }
  ...
  LIBUSB_InitInBuffer();
  if (rc < 0) {
    USBERR("[USBSID] Error, could not open device: %d, %s: %s\n", rc, libusb_error_name(rc), libusb_strerror(rc));
    goto out;
  }
...
int USBSID_Class::LIBUSB_Exit(void)
{
  ...
  rc = -1;
  devh = NULL;
  USBDBG("[USBSID] Closed USB device\n");
  return 0;
}
...
void EMSCRIPTEN_KEEPALIVE LIBUSB_CALL NOINLINE USBSID_Class::usb_out(struct libusb_transfer *transfer)
{
  if (transfer->status != LIBUSB_TRANSFER_COMPLETED) {
    ...
    if (rc != LIBUSB_TRANSFER_CANCELLED) {
      USBERR("[USBSID] Warning: transfer out interrupted with status %d, %s: %s\r", rc, libusb_error_name(rc), libusb_strerror(rc));
    }
  ...
  if (transfer->actual_length != len_out_buffer) {
    USBERR("[USBSID] Sent data length %d is different from the defined buffer length: %d or actual length %d\r", transfer->length, len_out_buffer, transfer->actual_length);
  }
...
void EMSCRIPTEN_KEEPALIVE LIBUSB_CALL NOINLINE USBSID_Class::usb_in(struct libusb_transfer *transfer)
{
  ...
  if (transfer->status != LIBUSB_TRANSFER_COMPLETED) {
    ...
    if (rc != LIBUSB_TRANSFER_CANCELLED) {
      USBERR("[USBSID] Warning: transfer in interrupted with status '%s'\r", libusb_error_name(rc));
    }
    libusb_free_transfer(transfer);
    return;
  }
