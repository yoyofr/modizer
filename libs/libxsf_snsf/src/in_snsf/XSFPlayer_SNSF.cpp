...
const char *XSFPlayerSNSF::WinampDescription = "SNSF Decoder";
const char *XSFPlayerSNSF::WinampExts = "snsf;minisnsf\0SNES Sound Format files (*.snsf;*.minisnsf)\0";

extern XSFConfigSNSF *xSFConfigSNSF;

XSFPlayerSNSF *XSFPlayerSNSF::Create(const std::string &fn)
{
	return new XSFPlayer_SNSF(fn);
}

#ifdef _WIN32
XSFPlayerSNSF *XSFPlayerSNSF::Create(const std::wstring &fn)
{
	return new XSFPlayer_SNSF(fn);
}
#endif
...
static bool Map2SF(XSFFileSNSF *xSF)
{
...
static bool RecursiveLoad2SF(XSFFileSNSF *xSF, int level)
{
	if (level <= 10 && xSF->GetTagExists("_lib"))
	{
#ifdef _WIN32
		auto libxSF = std::unique_ptr<XSFFileSNSF>(new XSFFileSNSF(ConvertFuncs::StringToWString(ExtractDirectoryFromPath(xSF->GetFilename()) + xSF->GetTagValue("_lib")), 4, 8));
#else
		auto libxSF = std::unique_ptr<XSFFileSNSF>(new XSFFileSNSF(ExtractDirectoryFromPath(xSF->GetFilename()) + xSF->GetTagValue("_lib"), 4, 8));
#endif
	...
	{
		found = false;
		std::string libTag = "_lib" + stringifySNSF(n++);
		if (xSF->GetTagExists(libTag))
		{
			found = true;
#ifdef _WIN32
			auto libxSF = std::unique_ptr<XSFFileSNSF>(new XSFFileSNSF(ConvertFuncs::StringToWString(ExtractDirectoryFromPath(xSF->GetFilename()) + xSF->GetTagValue(libTag)), 4, 8));
#else
			auto libxSF = std::unique_ptr<XSFFileSNSF>(new XSFFileSNSF(ExtractDirectoryFromPath(xSF->GetFilename()) + xSF->GetTagValue(libTag), 4, 8));
#endif
			if (!RecursiveLoad2SF(libxSF.get(), level + 1))
				return false;
		}
...
static bool Load2SF(XSFFileSNSF *xSF)
{
...
XSFPlayer_SNSF::XSFPlayer_SNSF(const std::string &filename) : XSFPlayerSNSF()
{
	this->xSF.reset(new XSFFileSNSF(filename, 4, 8));
}

#ifdef _WIN32
XSFPlayer_SNSF::XSFPlayer_SNSF(const std::wstring &filename) : XSFPlayerSNSF()
{
	this->xSF.reset(new XSFFileSNSF(filename, 4, 8));
}
#endif
...
bool XSFPlayer_SNSF::Load()
{
	...
	S9xInitAPU();
	XSFConfig_SNSF *xSFConfig_SNSF = dynamic_cast<XSFConfig_SNSF *>(xSFConfigSNSF);
