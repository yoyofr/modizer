...
static INLINE u16 T1ReadWord(u8 * mem, u32 addr)
{
	...
	// big endian handling (most significant byte first):
	// this impl does exactely the opposite!
   return (((u32)mem[addr + 1]) << 8) | mem[addr];
#else
	
#ifdef EMSCRIPTEN
	// assume the above incorrect WORDS_BIGENDIAN impl has never been used/tested - 
	// so just use the correct LE impl here
   return (((u32)mem[addr + 1]) << 8) | mem[addr];
#else
   return *((u16 *) (mem + addr));
#endif
...
static INLINE u32 T1ReadLong(u8 * mem, u32 addr)
{
   ...
   return (mem[addr + 3] << 24 | mem[addr + 2] << 16 |
	...
	// assume the above incorrect WORDS_BIGENDIAN impl has never been used/tested - 
	// so just use the correct LE impl here
   return (((u32)mem[addr + 3]) << 24 | ((u32)mem[addr + 2]) << 16 |
           ((u32)mem[addr + 1]) << 8 | mem[addr]);
#else
...
static INLINE u64 T1ReadQuad(u8 * mem, u32 addr)
{
   ...
   return (((u64)mem[addr + 7]) << 56 | ((u64)mem[addr + 6]) << 48 |
           ...
           ((u64)mem[addr + 1]) << 8 | mem[addr]);
#else
   return *((u64 *) (mem + addr));	// FIXME why the addr shifting in above func but NOT here??
#endif
...
#ifdef EMSCRIPTEN
	/*
	// do the inverse of the above incorrect impl?
   mem[addr + 0] = val >> 24;
...
#ifndef EMSCRIPTEN /* UNUSED CODE BELOW */
