					...
					*rmix++ +=(dt*rvol)>>(5+5);
                    ...
                    if (m_voice_ofs>=0) {
                        ...
                        if (ofs_end>ofs_start)
                            for (;;) {
                                m_voice_buff[m_voice_ofs+i][(ofs_start>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)&(SOUND_BUFFER_SIZE_SAMPLE*4*2-1)]=LIMIT8(((dt*(lvol+rvol))>>13));
				...
				for(j=0;j<samples;j++)
				{
					...
					*rmix++ +=(dt*rvol)>>5;
                    ...
                    if (m_voice_ofs>=0) {
                        ...
                        for (;;) {
                            m_voice_buff[m_voice_ofs+i][(ofs_start>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)&(SOUND_BUFFER_SIZE_SAMPLE*4*2-1)]=LIMIT8(((dt*(lvol+rvol))>>8));
                            ofs_start+=1<<MODIZER_OSCILLO_OFFSET_FIXEDPOINT;
                            if (ofs_start>=ofs_end) break;
                        }
                        while ((ofs_end>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)>=SOUND_BUFFER_SIZE_SAMPLE*4*2) ofs_end-=(SOUND_BUFFER_SIZE_SAMPLE*4*2<<MODIZER_OSCILLO_OFFSET_FIXEDPOINT);
