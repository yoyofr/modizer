...
void gameboy_update(UINT8 ChipID, stream_sample_t **outputs, int samples)
{
	...
	for (i = 0; i < samples; i++)
	{
        ...
        if (m_voice_ofs>=0) {
            ...
            for (;;) {
                if (gb->snd_1.on && !gb->snd_1.Muted)
                    m_voice_buff[m_voice_ofs+0][(ofs_start>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)&(SOUND_BUFFER_SIZE_SAMPLE*4*2-1)]=LIMIT8(((sample*(int)(gb->snd_control.vol_left+gb->snd_control.vol_right))>>2));
        ...
        //TODO:  MODIZER changes start / YOYOFR
        if (m_voice_ofs>=0) {
            int64_t ofs_start=m_voice_current_ptr[m_voice_ofs+1];
            int64_t ofs_end=(m_voice_current_ptr[m_voice_ofs+1]+smplIncr);
            if (ofs_end>ofs_start)
            for (;;) {
                if (gb->snd_2.on && !gb->snd_2.Muted)
                    m_voice_buff[m_voice_ofs+1][(ofs_start>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)&(SOUND_BUFFER_SIZE_SAMPLE*4*2-1)]=LIMIT8(((sample*(int)(gb->snd_control.vol_left+gb->snd_control.vol_right))>>2));
                ofs_start+=1<<MODIZER_OSCILLO_OFFSET_FIXEDPOINT;
                if (ofs_start>=ofs_end) break;
            }
            while ((ofs_end>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)>=SOUND_BUFFER_SIZE_SAMPLE*4*2) ofs_end-=(SOUND_BUFFER_SIZE_SAMPLE*4*2<<MODIZER_OSCILLO_OFFSET_FIXEDPOINT);
        ...
        //TODO:  MODIZER changes start / YOYOFR
        if (m_voice_ofs>=0) {
            int64_t ofs_start=m_voice_current_ptr[m_voice_ofs+2];
            int64_t ofs_end=(m_voice_current_ptr[m_voice_ofs+2]+smplIncr);
            if (ofs_end>ofs_start)
            for (;;) {
                if (gb->snd_3.on && !gb->snd_3.Muted)
                    m_voice_buff[m_voice_ofs+2][(ofs_start>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)&(SOUND_BUFFER_SIZE_SAMPLE*4*2-1)]=LIMIT8(((sample*(int)(gb->snd_control.vol_left+gb->snd_control.vol_right))>>2));
                ofs_start+=1<<MODIZER_OSCILLO_OFFSET_FIXEDPOINT;
                if (ofs_start>=ofs_end) break;
            }
            while ((ofs_end>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)>=SOUND_BUFFER_SIZE_SAMPLE*4*2) ofs_end-=(SOUND_BUFFER_SIZE_SAMPLE*4*2<<MODIZER_OSCILLO_OFFSET_FIXEDPOINT);
        ...
        if (m_voice_ofs>=0) {
            ...
            for (;;) {
                if (gb->snd_4.on && !gb->snd_4.Muted)
                    m_voice_buff[m_voice_ofs+3][(ofs_start>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)&(SOUND_BUFFER_SIZE_SAMPLE*4*2-1)]=LIMIT8(((sample*(int)(gb->snd_control.vol_left+gb->snd_control.vol_right))>>2));
                ofs_start+=1<<MODIZER_OSCILLO_OFFSET_FIXEDPOINT;
                if (ofs_start>=ofs_end) break;
            }
            while ((ofs_end>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)>=SOUND_BUFFER_SIZE_SAMPLE*4*2) ofs_end-=(SOUND_BUFFER_SIZE_SAMPLE*4*2<<MODIZER_OSCILLO_OFFSET_FIXEDPOINT);
