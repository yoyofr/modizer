  ...
  public:
    ...
    virtual void ExploreFiles(const Formats::Archived::Container::Walker& walker) const
    {
      for (uint_t idx = 0, total = CountFiles(); idx < total; ++idx)
      {
        ...
        {
#ifdef EMSCRIPTENX
			// for some reason the Text::AY_FILENAME_PREFIX based impl fails
          const String subPath = Filename(String("#"), idx).ToString();
#else
    ...
    {
#ifdef EMSCRIPTENX
      const Filename rawName(String("@"), name);
      const Filename ayName(String("#"), name);
#else
...
namespace Formats
{
  namespace Archived
  {
    ...
    public:
      ...
      {
        return String(Text::AY_ARCHIVE_DECODER_DESCRIPTION);
      }
