...
#pragma pack(push,1)
    ...
    {
      RawId Id;
      ruint16_t SamplesStart;
      ruint16_t PositionsOffset;
      uint8_t Tempo;
      ruint16_t OrnamentsTableOffset;
    } PACK_POST;
	
    PACK_PRE struct RawInt16
    {
      ruint16_t Val;		// need to wrap this to allow unaligned access..
    } PACK_POST;
    ...
    PACK_PRE struct RawPattern
    {
      uint8_t Index;
      uint8_t Size;
      ruint16_t Offsets[3];//from start of patterns
    ...
    public:
      ...
      void ParseSamples(const Indices& samples, Builder& builder) const
      {
        ...
        for (Indices::Iterator it = samples.Items(); it; ++it)
        {
          ...
          const std::size_t sampleAddr = Trait.SamplesBase + fromLE(GetServiceObject<RawInt16>(offsetAddr).Val);
      ...
      void ParseOrnaments(const Indices& ornaments, Builder& builder) const
      {
        ...
        for (Indices::Iterator it = ornaments.Items(); it; ++it)
        {
          ...
          if (ornIdx < maxOrnaments)
          {
            ...
            const std::size_t ornamentAddr = Trait.OrnamentsBase + fromLE(GetServiceObject<RawInt16>(offsetAddr).Val);
...
//          std::transform(pat.Offsets.begin(), pat.Offsets.end(), begin(), &fromLE<uint16_t>);	  
		  int i;
		  for (i= 0; i<size(); i++) {
			at(i)= fromLE(pat.Offsets[i]);	// unaligned int16_t values must be accessed via "RawPattern"
		  }
        }
      };
    ...
    struct Areas : public AreaController
    {
      Areas(const Binary::TypedContainer& data)
      {
        ...
        if (const RawInt16* firstSample = data.GetField<RawInt16>(samplesOffsets))
        {
          const std::size_t firstSampleStart = fromLE(firstSample->Val) + traits.SamplesBase;
          if (firstSampleStart == std::size_t(fromLE(header.SamplesStart) + 1))
          {
            AddArea(SAMPLES, firstSampleStart);
          }
        }
        if (const ruint16_t* firstOrnament = &(data.GetField<RawInt16>(ornamentsOffsets)->Val))
        {
