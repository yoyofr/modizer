...
#pragma pack(push,1)
    ...
    {
      ruint16_t Offsets[MAX_ORNAMENTS_COUNT];
    } PACK_POST;
    ...
    {
      ruint16_t Offsets[MAX_SAMPLES_COUNT];
    } PACK_POST;
#ifdef USE_PRAGMA_PACK
#pragma pack(pop)
#endif
    ...
    private:
      ...
      {
        const uint16_t offset = fromLE(GetServiceObject<RawInt16>(index, fromLE(Source.SamplesOffset)).Val) - UnfixDelta;
      ...
      {
        const uint16_t offset = fromLE(GetServiceObject<RawInt16>(index, fromLE(Source.OrnamentsOffset)).Val) - UnfixDelta;
        const RawObject* const obj = Delegate.GetField<RawObject>(offset);
        Require(obj != 0);
