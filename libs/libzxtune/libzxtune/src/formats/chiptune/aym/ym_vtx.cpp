...
#pragma pack(push,1)
#endif
    typedef uint8_t RegistersDump[14];
    typedef uint8_t IdentifierType[4];
    ...
    namespace Compressed
    {
      PACK_PRE struct RawHeader
      {
        ...
        uint8_t HeaderSum;
        char Method[5];
        ruint32_t PackedSize;
        ruint32_t OriginalSize;
        ruint32_t Time;
        uint8_t Attribute;
        uint8_t Level;
        uint8_t NameLen;
...
#pragma pack(pop)
    ...
    Formats::Chiptune::Container::Ptr ParseVTX(const Binary::Container& rawData, Builder& target)
    {
      ...
      try
      {
        ...
        if (newVersion)
        {
          target.SetYear(fromLE(stream.ReadField<RawInt16>().Val));
        }
        const uint_t unpackedSize = fromLE(stream.ReadField<RawInt32>().Val);
        target.SetTitle(FromStdString(stream.ReadCString(MAX_STRING_SIZE)));
        target.SetAuthor(FromStdString(stream.ReadCString(MAX_STRING_SIZE)));
        if (newVersion)
        {
