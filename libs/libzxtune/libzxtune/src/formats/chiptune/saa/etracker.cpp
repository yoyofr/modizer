...
#pragma pack(push,1)
    ...
    PACK_PRE struct RawHeader
    {
      ...
      ruint16_t SamplesVolumeDecodeTable;
...
#pragma pack(pop)
    ...
    private:
      ...
      {
        const std::size_t size = sizeof(ruint16_t);
        const std::size_t offset = base + idx * size;
        const RawUint16* const data = Delegate.GetField<RawUint16>(offset);
        Require(data != 0);
        Ranges.AddService(offset, size);
        return fromLE(data->Val);	// potentially unaligned read
      }
