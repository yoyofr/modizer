...
#pragma pack(push,1)
    ...
    PACK_PRE struct Header
    {
      ...
      uint8_t Banks[0x40];
      //+0xc0
      LayoutInfo Layouts[LAYOUT_INFO_SIZE];
      //+0xe0
      uint8_t Padding1[0x20];
      //+0x100
      boost::array<char, 0x20> Title;	// who cares if this is unaligned..
      //+0x120
      ...
      //+0x300
      uint8_t SampleNames[8][SAMPLES_COUNT];
      //+0x380
...
//        const std::vector<uint_t> positions(Source.Positions.begin(), Source.Positions.begin() + Source.LastPosition + 1);
        std::vector<uint_t> positions(Source.LastPosition + 1);
		for (int i= 0; i<Source.LastPosition + 1; i++) {
      ...
      void ParseSamples(const Indices& sams, Builder& target) const
      {
        ...
        for (std::size_t layIdx = 0, cursor = sizeof(Source); layIdx != LAYOUT_INFO_SIZE; ++layIdx)
        {
