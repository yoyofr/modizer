...
#pragma pack(push,1)
    ...
    typedef Note Line[CHANNELS_COUNT];

    typedef Line Pattern[PATTERN_SIZE];

    PACK_PRE struct Header
    {
      Ornament Ornaments[ORNAMENTS_COUNT];		// 11*16	->176
      OrnamentLoop OrnLoops[ORNAMENTS_COUNT];	// 11*2		-> 22 
      uint8_t Padding1[6];						//				6
      char Title[32];							//				32
      uint8_t Tempo;
      uint8_t Start;
      uint8_t Loop;
      uint8_t Length;							// 				4
      uint8_t Padding2[16];						//				16
	  // offset: 265		0x100
      Sample Samples[SAMPLES_COUNT];
      uint8_t Positions[POSITIONS_COUNT];
      ruint16_t LastDatas[PAGES_COUNT];
      uint8_t FreeRAM;
      uint8_t Padding3[5];
