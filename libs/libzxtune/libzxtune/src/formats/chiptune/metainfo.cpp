...
#else 
// we may be dealing with unaligned data.. therefore the above is useless..
  void FixUnaligned(RawUInt16* data, int_t delta)
  {
    unsigned short fixedVal = fromLE(data->Val) + delta;
    data->Val = fromLE(fixedVal);
  }
#endif
...
#else
        FixUnaligned((RawUInt16*)(&result[it->first]), it->second);
#endif
      }
    }
