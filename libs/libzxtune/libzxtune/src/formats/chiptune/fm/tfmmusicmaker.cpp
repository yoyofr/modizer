...
#pragma pack(push,1)
    ...
    typedef uint8_t InstrumentName[16];
    ...
    {
      CellType Channels[CHANNELS_COUNT];
    ...
    struct Version05
    {
      ...
      {
		uint8_t Notes[MAX_PATTERN_SIZE];
        uint8_t Volumes[MAX_PATTERN_SIZE];
        uint8_t Instruments[MAX_PATTERN_SIZE];
        uint8_t Effects[MAX_PATTERN_SIZE];
        uint8_t EffectParameters[MAX_PATTERN_SIZE];
      ...
      PACK_PRE struct RawHeader
      {
        ...
        InstrumentName InstrumentNames[MAX_INSTRUMENTS_COUNT];
        RawInstrument Instruments[MAX_INSTRUMENTS_COUNT];
    ...
    struct Version13
    {
      ...
      PACK_PRE struct RawCell
      {
        struct RawEffect
        {
          uint8_t Code[MAX_PATTERN_SIZE];
          uint8_t Parameter[MAX_PATTERN_SIZE];
        };

        uint8_t Notes[MAX_PATTERN_SIZE];
        uint8_t Volumes[MAX_PATTERN_SIZE];
        uint8_t Instruments[MAX_PATTERN_SIZE];
        RawEffect Effects[EFFECTS_COUNT];
        ...
        void GetCell(uint_t line, Cell& result) const
        {
          ...
          result.Volume = Volumes[line];
          result.Instrument = Instruments[line];
          for (uint_t eff = 0; eff != EFFECTS_COUNT; ++eff)
          {
