...
#pragma pack(push,1)
  ...
  {
    uint8_t Id[4];          //'RIFF'
    ruint32_t Size;          //file size - 8
    uint8_t Type[4];        //'WAVE'
    uint8_t ChunkId[4];     //'fmt '
    ruint32_t ChunkSize;     //16
    ruint16_t Compression;   //1
    ruint16_t Channels;
    ruint32_t Samplerate;
    ruint32_t BytesPerSec;
    ruint16_t Align;
    ruint16_t BitsPerSample;
  ...
  {
    uint8_t Id[4];   //LIST
    ruint32_t Size;   //next content size
    uint8_t Type[4]; //INFO
  } PACK_POST;
  ...
  {
    uint8_t Id[4];
    ruint32_t Size; //next content size - 1
    uint8_t Content[1];
  } PACK_POST;
#ifdef USE_PRAGMA_PACK
#pragma pack(pop)
