...
bool DivEngine::load(unsigned char* f, size_t slen, const char* nameHint) {
  ...
  try {
    ...
    if (!inflateEnded) nextErr=inflateEnd(&zl); else nextErr=Z_OK;
    if (nextErr!=Z_OK) {
      if (zl.msg==NULL) {
