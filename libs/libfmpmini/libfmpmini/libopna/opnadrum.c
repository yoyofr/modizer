        ...
        for (int d = 0; d < 6; d++) {
            if (drum->drums[d].playing && drum->drums[d].data) {
                ...
                if (!(drum->mask & (1u << d))) {
                    ...
                    if (drum->drums[d].left||drum->drums[d].right) smpl=co; //YOYOFR
                }
                
                //TODO:  MODIZER changes start / YOYOFR
                int64_t ofs_start=m_voice_current_ptr[d+6+3];
                int64_t ofs_end=(m_voice_current_ptr[d+6+3]+smplIncr);
                
                for (;;) {
                    m_voice_buff[d+6+3][(ofs_start>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)&(SOUND_BUFFER_SIZE_SAMPLE*2*4-1)]=LIMIT8(((smpl)>>4));
                ...
                m_voice_current_ptr[d+6+3]=ofs_end;
                //TODO:  MODIZER changes end / YOYOFR
