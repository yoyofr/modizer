...
void ppz8_mix(struct ppz8 *ppz8, int16_t *buf, unsigned samples) {
    ...
    //YOYOFR
    for (int i=16;i<m_genNumVoicesChannels;i++) {
        vgm_last_note[i]=0;
        vgm_last_vol[i]=0;
    }
    m_voice_current_system=16;
    for (int i=0;i<8;i++) {
        if ( !((1u << i) & ppz8->mask) && ppz8->channel[i].playing && ppz8->channel[i].vol ) {
    ...
    for (unsigned i = 0; i < samples; i++) {
        ...
        for (int p = 0; p < 8; p++) {
            ...
            if (!channel->playing) {
                //TODO:  MODIZER changes start / YOYOFR
                int64_t ofs_end=(m_voice_current_ptr[16+p]+smplIncr);
                m_voice_current_ptr[16+p]=ofs_end;
                //TODO:  MODIZER changes end / YOYOFR
                continue;
            }
            ...
            //TODO:  MODIZER changes start / YOYOFR
            int64_t ofs_start=m_voice_current_ptr[16+p];
            int64_t ofs_end=(m_voice_current_ptr[16+p]+smplIncr);
            int smpl=out*MAXVAL(pan_vol[channel->pan][0],pan_vol[channel->pan][1])>>2;
            for (;;) {
                m_voice_buff[16+p][(ofs_start>>MODIZER_OSCILLO_OFFSET_FIXEDPOINT)&(SOUND_BUFFER_SIZE_SAMPLE*2*4-1)]=LIMIT8(((smpl)>>6));
