...
void CustomWaveform::Draw(const PerFrameContext& presetPerFrameContext)
{
    ...
    const float t = m_spectrum ? (static_cast<float>(maxSampleCount - m_sep) / static_cast<float>(sampleCount)) : 1;
    ...
    shader->SetUniformFloat("vertex_point_size", (m_drawThick?2.0:1.0));
