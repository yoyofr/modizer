...
class AltMilkdropPreset : public ::libprojectM::Preset
{

public:
    /**
     * @brief LoadCode a AltMilkdropPreset by filename with input and output buffers specified.
     * @param factory The factory class that created this preset instance.
     * @param absoluteFilePath the absolute file path of a AltMilkdropPreset to load from the file system
     */
    AltMilkdropPreset(const std::string& absoluteFilePath);

    /**
     * @brief LoadCode a AltMilkdropPreset from an input stream with input and output buffers specified.
     * @param presetData an already initialized input stream to read the AltMilkdropPreset file from
     * @param presetOutputs initialized and filled with data parsed from a AltMilkdropPreset
     */
    AltMilkdropPreset(std::istream& presetData);
...
private:
    ...
    std::string m_absoluteFilePath; //!< The absolute file path of the AltMilkdropPreset
    std::string m_absolutePath;     //!< The absolute path of the AltMilkdropPreset
