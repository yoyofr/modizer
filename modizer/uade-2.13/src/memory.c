...
void memory_deinit(void) {
	if (kickmemory) {free(kickmemory);kickmemory=NULL;}
	if (chipmemory) {free(chipmemory);chipmemory=NULL;}
	
	if (bogomemory) {free(bogomemory);bogomemory=NULL;}
	if (a3000memory) {free(a3000memory);a3000memory=NULL;}
}
