...
protected:
	...
	std::unique_ptr<XSFFileSNSF> xSF;
	...
	XSFPlayerSNSF();
	XSFPlayerSNSF(const XSFPlayerSNSF &xSFPLayer);
public:
	...
	static XSFPlayerSNSF *Create(const std::string &fn);

	virtual ~XSFPlayerSNSF() { }
	XSFPlayerSNSF &operator=(const XSFPlayerSNSF &xSFPlayer);
	XSFFileSNSF *GetXSFFileSNSF() { return this->xSF.get(); }
	const XSFFileSNSF *GetXSFFileSNSF() const { return this->xSF.get(); }
