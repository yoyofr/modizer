...
XSFFileSNSF::XSFFileSNSF() : xSFType(0), hasFile(false), rawData(), reservedSection(), programSection(), tags(), fileName("")
{
}

XSFFileSNSF::XSFFileSNSF(const std::string &filename) : xSFType(0), hasFile(false), rawData(), reservedSection(), programSection(), tags(), fileName(filename)
{
	this->ReadXSF(filename, 0, 0, true);
}

XSFFileSNSF::XSFFileSNSF(const std::string &filename, uint32_t programSizeOffset, uint32_t programHeaderSize) : xSFType(0), hasFile(false), rawData(), reservedSection(), programSection(), tags(), fileName(filename)
{
	this->ReadXSF(filename, programSizeOffset, programHeaderSize);
}


void XSFFileSNSF::ReadXSF(const std::string &filename, uint32_t programSizeOffset, uint32_t programHeaderSize, bool readTagsOnly)
{
...
void XSFFileSNSF::ReadXSF(std::ifstream &xSF, uint32_t programSizeOffset, uint32_t programHeaderSize, bool readTagsOnly)
{
...
bool XSFFileSNSF::IsValidType(uint8_t type) const
{
	return this->xSFType == type;
}

void XSFFileSNSF::Clear()
{
	this->xSFType = 0;
	this->hasFile = false;
...
bool XSFFileSNSF::HasFile() const
{
	return this->hasFile;
}

std::vector<uint8_t> &XSFFileSNSF::GetReservedSection()
{
	return this->reservedSection;
}

std::vector<uint8_t> XSFFileSNSF::GetReservedSection() const
{
	return this->reservedSection;
}

std::vector<uint8_t> &XSFFileSNSF::GetProgramSection()
{
	return this->programSection;
}

std::vector<uint8_t> XSFFileSNSF::GetProgramSection() const
{
	return this->programSection;
}

const TagListSNSF &XSFFileSNSF::GetAllTags() const
{
	return this->tags;
}

void XSFFileSNSF::SetAllTags(const TagListSNSF &newTags)
{
	this->tags = newTags;
}

void XSFFileSNSF::SetTag(const std::string &name, const std::string &value)
{
	this->tags[name] = value;
}

void XSFFileSNSF::SetTag(const std::string &name, const std::wstring &value)
{
	this->tags[name] = ConvertFuncsSNSF::WStringToString(value);
}

bool XSFFileSNSF::GetTagExists(const std::string &name) const
{
	return this->tags.Exists(name);
}

std::string XSFFileSNSF::GetTagValue(const std::string &name) const
{
	return this->GetTagExists(name) ? this->tags[name] : "";
}

unsigned long XSFFileSNSF::GetLengthMS(unsigned long defaultLength) const
{
	unsigned long length = 0;
	std::string value = this->GetTagValue("length");
	if (!value.empty())
		length = ConvertFuncsSNSF::StringToMS(value);
	if (!length)
		length = defaultLength;
	return length;
}

unsigned long XSFFileSNSF::GetFadeMS(unsigned long defaultFade) const
{
	unsigned long fade = defaultFade;
	std::string value = this->GetTagValue("fade");
	if (!value.empty())
		fade = ConvertFuncsSNSF::StringToMS(value);
	return fade;
}

double XSFFileSNSF::GetVolume(VolumeTypeSNSF preferredVolumeType, PeakTypeSNSF preferredPeakType) const
{
	if (preferredVolumeType == VOLUMETYPESNSF_NONE)
		return 1.0;
	...
	if (preferredVolumeType == VOLUMETYPESNSF_REPLAYGAIN_ALBUM && !replaygain_album_gain.empty())
	{
		gain = convertToSNSF<double>(replaygain_album_gain, false);
		hadReplayGain = true;
	}
	if (!hadReplayGain && preferredVolumeType != VOLUMETYPESNSF_VOLUME && !replaygain_track_gain.empty())
	{
		gain = convertToSNSF<double>(replaygain_track_gain, false);
		hadReplayGain = true;
	}
	if (hadReplayGain)
	{
		double vol = std::pow(10.0, gain / 20.0), peak = 1.0;
		if (preferredPeakType == PEAKTYPESNSF_REPLAYGAIN_ALBUM && !replaygain_album_peak.empty())
			peak = convertToSNSF<double>(replaygain_album_peak, false);
		else if (preferredPeakType != PEAKTYPESNSF_NONE && !replaygain_track_peak.empty())
			peak = convertToSNSF<double>(replaygain_track_peak, false);
		return !fEqual(peak, 1.0) ? std::min(vol, 1.0 / peak) : vol;
	}
	return volume.empty() ? 1.0 : convertToSNSF<double>(volume, false);
}

std::string XSFFileSNSF::FormattedTitleOptionalBlock(const std::string &block, bool &hadReplacement, unsigned level) const
{
...
std::string XSFFileSNSF::GetFormattedTitle(const std::string &format) const
{
...
std::string XSFFileSNSF::GetFilename() const
{
	return this->fileName;
}

std::string XSFFileSNSF::GetFilenameWithoutPath() const
{
	return ExtractFilenameFromPath(this->fileName);
}

void XSFFileSNSF::SaveFile() const
{
...
#ifdef _WIN32
	xSF.open(ConvertFuncsSNSF::StringToWString(this->fileName).c_str(), std::ofstream::out | std::ofstream::binary);
#else
