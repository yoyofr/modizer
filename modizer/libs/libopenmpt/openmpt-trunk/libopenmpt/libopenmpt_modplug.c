...
#else
#include "include/modplug/include/libmodplug/modplug.h"
#endif
...
LIBOPENMPT_MODPLUG_API ModPlugFile* ModPlug_Load(const void* data, int size)
{
	...
	file->buf = (short*)malloc(BUFFER_COUNT*sizeof(signed short)*4);
	if(!file->buf){
	...
	if(name){
		...
		file->name = (char*)malloc(strlen("")+1);
		if(file->name){
			strcpy(file->name,"");
		}
	}
	...
	if(message){
		file->message = (char*)malloc(strlen(message)+1);
		if(file->message){
		...
		file->message = (char*)malloc(strlen("")+1);
		if(file->message){
			strcpy(file->message,"");
		}
	}
...
LIBOPENMPT_MODPLUG_API const char *ModPlug_GetModuleTypeLStr(ModPlugFile* file){
    const char* type;
    if(!file) return NULL;
    type = openmpt_module_get_metadata(file->mod,"type_long");
    return type;
}

LIBOPENMPT_MODPLUG_API const char *ModPlug_GetModuleContainerLStr(ModPlugFile* file){
    const char* container;
    if(!file) return NULL;
    container = openmpt_module_get_metadata(file->mod,"container_long");
    return container;
}
...
LIBOPENMPT_MODPLUG_API ModPlugNote* ModPlug_GetPattern(ModPlugFile* file, int pattern, unsigned int* numrows)
{
	...
	if(!file->patterns){
		file->patterns = (ModPlugNote**)malloc(sizeof(ModPlugNote*)*openmpt_module_get_num_patterns(file->mod));
		if(!file->patterns) return NULL;
		memset(file->patterns,0,sizeof(ModPlugNote*)*openmpt_module_get_num_patterns(file->mod));
	}
	if(!file->patterns[pattern]){
		file->patterns[pattern] = (ModPlugNote*)malloc(sizeof(ModPlugNote)*openmpt_module_get_pattern_num_rows(file->mod,pattern)*openmpt_module_get_num_channels(file->mod));
...
LIBOPENMPT_MODPLUG_API void ModPlug_InitMixerCallback(ModPlugFile* file,ModPlugMixerProc proc)
{
	if(!file) return;
	if(!file->mixerbuf){
		file->mixerbuf = (int*)malloc(BUFFER_COUNT*sizeof(signed int)*4);
	}
	file->mixerproc = proc;
}
