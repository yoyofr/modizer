...
// Is this necessary psxMemReadSX?
...
extern u32 seektime,sampcount,sexy_shouldstop;

static void intExecute() {
	for (;;) {
		if(!sputhingy()) {
			psxShutdown();
			if ((seektime!=-1)&&(!sexy_shouldstop)) {
				if(seektime<sampcount) {
					//need to restart
					int t=seektime;
					/*int r=*/sexy_reload();
					seektime=t;
//					printf("reload %d, seektime %d, sampcount %d",r,seektime,);
				}
			} else return;
		}
		flushboot();
