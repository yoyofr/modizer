...
ymbool	CYmMusic::updateStereo(ymsample *sampleBuffer,ymint nbSample)
{
	ymint sampleToCompute;
	ymint	vblNbSample;
	...
	if ((!bMusicOk) ||
		...
		(bMusicOver))
	{
		bufferClear(sampleBuffer,nbSample*2);
		if (bMusicOver)
			return YMFALSE;
		else
			return YMTRUE;
	...
	if ((songType >= YM_MIX1) && (songType < YM_MIXMAX))
	{
		stDigitMixStereo(sampleBuffer,nbSample);
	}
	else if ((songType >= YM_TRACKER1) && (songType<YM_TRACKERMAX))
	{
		ymTrackerUpdateStereo(sampleBuffer,nbSample);
	}
	else
	{
		ymsample *pOut = sampleBuffer;
...
void	CYmMusic::stDigitMixStereo(ymsample *pWrite16,ymint nbs)
{
...
void	CYmMusic::ymTrackerVoiceAddStereo(ymTrackerVoice_t *pVoice,ymsample *pBuffer,ymint nbs)
{
	ymsample *pVolumeTab;
	ymu8 *pSample;
	ymu32 samplePos;
...
void	CYmMusic::ymTrackerUpdateStereo(ymsample *pBuffer,ymint nbSample)
{
	ymint i;
	ymint _nbs;
	...
	do
	{
		...
		if (_nbs>0)
		{
			...
			for (i=0;i<nbVoice;i++)
			{
				ymTrackerVoiceAddStereo(&ymTrackerVoice[i],pBuffer,_nbs);
			}
			pBuffer += _nbs*2;
			nbSample -= _nbs;
		}
