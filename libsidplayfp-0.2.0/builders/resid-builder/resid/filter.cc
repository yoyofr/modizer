...
static float_point opamp_voltage_6581[] = {
...
static float_point opamp_voltage_8580[] = {
...
Filter::Filter()
{
  ...
  if (!class_init) {
    ...
    for (int m = 0; m < 2; m++) {
      ...
      // points.
      // float_point scaled_voltage[fi.opamp_voltage_size];
      float_point scaled_voltage[50];
    ...
    float N16 = model_filter[0].vo_N16;
    float vmin = N16*fi.opamp_voltage[0][0];
    float k = fi.k;
    ...
    */
    ...
    for (int kVg_Vx = 0; kVg_Vx < (1 << 16); kVg_Vx++) {
      float log_term = log(1 + exp((kVg_Vx/N16 - kVt)/(2*Ut)));
      // Scaled by m*2^15
