...
void build_dac_table(unsigned short* dac, int bits, float _2R_div_R, bool term)
{
  ...
  for (int set_bit = 0; set_bit < bits; set_bit++) {
    ...
    float Vn = 1.0;          // Normalized bit voltage.
    float R = 1.0;           // Normalized R
    float _2R = _2R_div_R*R; // 2R
