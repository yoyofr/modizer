...
- (int) mmp_wsrLoad:(NSString*)filePath {  //WSR
    ...
    if (m3uReader.size()) {
        ...
        for (uint32_t i = 0; i < 256; i++)
        {
            ...
            uint64_t smp = 0;
            auto numSamples = (48000 * 4) & ~(kBufSamples - 1); // around 4 seconds check
            while (numSamples)
            {
                s_coreSwan->pUpdateWSR(buf, sizeof(buf), kBufSamples);
                if (numSamples == ((48000 * 4) & ~(kBufSamples - 1)))
                    smp = buf[0];
    ...
    if (mod_subsongs>1) {
        ...
        for (int i=0;i<mod_subsongs;i++) {
            ...
            if (subsong_length<=0) {
                subsong_length=optGENDefaultLength;
            }
...
-(int) mmp_vgmplayLoad:(NSString*)filePath { //VGM
    ...
    for (curDev = 0; curDev < diList.size(); curDev ++) {
        ...
        if (firstChip) {
            sprintf(mod_message+strlen(mod_message),"Chipsets....: %s",mod_activeChipsName[mod_activeChipsNb]);
            firstChip=0;
        } else sprintf(mod_message+strlen(mod_message),",%s",mod_activeChipsName[mod_activeChipsNb]);
...
-(NSString*) getVoicesName:(unsigned int)channel onlyMidi:(bool)onlyMidi {
    ...
    switch (mPlayType) {
        ...
        case MMP_WSR:
            return [NSString stringWithFormat:@"#%d-WSR",channel+1];
        case MMP_PIXEL: {
            if (pixel_organya_mode) {
