...
-(void) fillKeysWithRepoCateg {
    ...
    for (int i=0;i<dbWEB_nb_entries;i++) {
        ...
        dbWEB_entries[dbWEB_entries_count].fullpath=[[NSString alloc] initWithFormat:@"%@",wentry->category];//@"VGMRips";
...
//                            } else we[we_index].file_systems=nil;
                            ...
                            if ([el hasChildren]) {
                                ...
                                for (int ii=0;ii<[el_ar count];ii++) {
                                    TFHppleElement *ela=[el_ar objectAtIndex:ii];
                                    if (system) system=[NSString stringWithFormat:@"%@,%@",system,[ela text]];
                                    else system=[NSString stringWithFormat:@"%@",[ela text]];
                                }
                            }
                            we[we_index].file_systems=[NSString stringWithFormat:@"%@",system];
...
- (void)tableView:(UITableView *)tabView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (cur_db_entries[indexPath.row].isFile) { //FILE
        ...
        ((RootViewControllerVGMRWebParser*)childController)->mWebBaseURL=cur_db_entries[indexPath.row].URL;
