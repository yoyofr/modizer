...
static DUMB_IT_SIGDATA *it_mod_load_sigdata(DUMBFILE *f, int _drestrict)
{
	...
	*/
	...
	// moo
	if ( _drestrict && sigdata->n_samples == 15 )
	{
		free(sigdata);
		dumbfile_close(f);
		return NULL;
...
DUH *dumb_read_mod_quick(DUMBFILE *f, int _drestrict)
{
	sigdata_t *sigdata;

	DUH_SIGTYPE_DESC *descptr = &_dumb_sigtype_it;

	sigdata = it_mod_load_sigdata(f, _drestrict);
